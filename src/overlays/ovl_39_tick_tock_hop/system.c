...
void o39_SortFuncGroup(FuncGroup* group);
...
void o39_UpdateColliders(FuncGroupData* groupData, FuncWork* work);
f32 o39_CalcSideOfEdge(QuadCollider* collider, ModelTracker* model, Vec* a, Vec* b);
void o39_MakeIdentityMtx(f32* out);
void o39_MakeTransformMtx(f32 transX, f32 transY, f32 transZ, f32 rotX, f32 rotY, f32 rotZ, f32* out);
void o39_ApplyTransformMtx(Vec* in, s16 n, f32* mtx, Vec* out);

BillboardData* BSS o39_billboards;
FuncGroup* BSS o39_funcGroups;
SpriteData* BSS o39_sprites;
AnimModelData* BSS o39_animModels;
QuadCollider* BSS o39_colliders;
...
u8 o39_quadEdges[][3] = { { 0, 1 }, { 1, 2 }, { 2, 3 }, { 3, 0 } };

void o39_CreateSystem(void) {
    o39_sprites = HuMemAllocTag(SPRITES_MAX * sizeof(*o39_sprites), 31000);
    memset(o39_sprites, 0, SPRITES_MAX * sizeof(*o39_sprites));
    o39_animModels = HuMemAllocTag(ANIMMDL_MAX * sizeof(*o39_animModels), 31000);
    memset(o39_animModels, 0, ANIMMDL_MAX * sizeof(*o39_animModels));
    o39_billboards = HuMemAllocTag(BILLS_MAX * sizeof(*o39_billboards), 31000);
    memset(o39_billboards, 0, BILLS_MAX * sizeof(*o39_billboards));
    o39_funcGroups = HuMemAllocTag(5 * sizeof(*o39_funcGroups), 31000);
}
...
FuncGroup* o39_CreateFuncGroup(omObjData* object, s8 groupId, s16 maxModels, s16 maxFuncs) {
    ...
    group = &o39_funcGroups[groupId];
    if (maxFuncs != 0) {
        group->funcWork = HuMemAllocTag(maxFuncs * sizeof(*group->funcWork), 31000);
        memset(group->funcWork, 0, maxFuncs * sizeof(*group->funcWork));
        group->order = HuMemAllocTag((maxFuncs + 1) * sizeof(*group->order), 31000);
    }
    group->maxFuncs = maxFuncs;
    group->data = HuMemAllocTag(sizeof(*group->data), 31000);
    if (maxModels != 0) {
        group->data->modelIds = HuMemAllocTag(maxModels * sizeof(*group->data->modelIds), 31000);
        memset(group->data->modelIds, 0, maxModels * sizeof(*group->data->modelIds));
    }
    group->data->maxModels = maxModels;
    group->sort = TRUE;
    group->data->object = object;
    return group;
}

s16 o39_SetFunc(s8 groupId, s32 prio, void* data, void (*func)(FuncGroupData*, FuncWork*), s8 tag) {
    FuncGroup* group = &o39_funcGroups[groupId];
    FuncWork* work = group->funcWork;
    s16 maxFuncs = group->maxFuncs;
    s16 i;

    for (i = 0; i < maxFuncs; i++, work++) {
        if (!work->set) {
            work->set = TRUE;
            work->prio = prio;
            work->id = i;
...
void o39_ResetFunc(s8 groupId, FuncWork* work) {
    ...
    o39_funcGroups[groupId].sort = TRUE;
}
...
void o39_ResetFuncTag(s8 groupId, s8 tag) {
    ...
    work = group->funcWork;
    for (i = 0; i < group->maxFuncs; i++, work++) {
        if (work->tag == tag) {
            o39_ResetFunc(groupId, work);
        }
    }
}
...
void o39_ResetFuncGroup(s8 groupId) {
    ...
    memset(group->funcWork, 0, group->maxFuncs * sizeof(*group->funcWork));
    group->sort = TRUE;
}
...
void o39_UpdateFuncGroup(s8 groupId) {
    ...
    if (group->sort) {
        o39_SortFuncGroup(group);
    }
    orderId = 0;
    while (group->order[orderId].next != -1) {
        orderId = group->order[orderId].next;
        work = &group->funcWork[group->order[orderId].workId];
        if (work->set) {
            work->func(group->data, work);
        }
    }
}
...
void o39_SortFuncGroup(FuncGroup* group) {
    ...
    FuncWork* currWork = group->funcWork;
    s16 orderId;
    s16 prevId;
    s16 i, j;
    ...
    order[0].next = -1;
    orderId = 1;
    for (i = 0; i < group->maxFuncs; i++, currWork++) {
        if (!currWork->set) {
            continue;
        }
        prevId = j = 0;
        while (order[j].next != -1) {
            j = order[j].next;
            if (order[j].prio < currWork->prio) {
                prevId = j;
                break;
            }
        }
        order[orderId].workId = i;
        order[orderId].prio = currWork->prio;
...
void o39_SetSpriteDispOn(s16 spriteId, s32 posX, s32 posY) {
    SpriteData* sprite = &o39_sprites[spriteId];
...
s32 o39_SetAnimModelDispOn(s16 animModelId, f32 posX, f32 posY, f32 posZ, f32 rotX, f32 rotY, f32 rotZ, f32 scale, f32 animStart) {
    ...
    if (!o39_animModels[animModelId].set) {
        return FALSE;
    }
    animModel = &o39_animModels[animModelId];
    hmfModel = &HmfModelData[animModel->modelId];
...
void o39_SetBillDispOn(s16 billId, f32 posX, f32 posY, f32 posZ) {
    if (billId < BILLS_MAX) {
        HmfModel* model = &HmfModelData[o39_billboards[billId].modelId];

        model->pos.x = posX;
        model->pos.y = posY;
        model->pos.z = posZ;
        func_8001C258_1CE58(o39_billboards[billId].modelId, 4, 0);
        o39_billboards[billId].attr &= ~(BILL_ATTR_DISPOFF | BILL_ATTR_REF);
    }
}
...
void o39_SetBillDispOnRef(s16 billId, f32* posX, f32* posY, f32* posZ) {
    if (billId < BILLS_MAX) {
        ...
        if (posX != NULL) {
            o39_billboards[billId].posX = posX;
        } else {
            o39_billboards[billId].posX = &model->pos.x;
        }
        if (posY != NULL) {
            o39_billboards[billId].posY = posY;
        } else {
            o39_billboards[billId].posY = &model->pos.y;
        }
        if (posZ != NULL) {
            o39_billboards[billId].posZ = posZ;
        } else {
            o39_billboards[billId].posZ = &model->pos.z;
        }
        func_8001C258_1CE58(o39_billboards[billId].modelId, 4, 0);
        o39_billboards[billId].attr &= ~(BILL_ATTR_DISPOFF | BILL_ATTR_REF);
        o39_billboards[billId].attr |= BILL_ATTR_REF;
    }
}
...
HmfModel* o39_GetBillModel(s16 billId) {
    ...
    return &HmfModelData[o39_billboards[billId].modelId];
}
...
void o39_UpdateBills(void) {
    ...
    for (i = 0; i < BILLS_MAX; i++, bill++) {
        ...
        model->rot.x = CRot.x;
        model->rot.y = CRot.y;
        o39_MakeIdentityMtx(model->unk74);
...
void o39_CreateColliders(FuncGroupData* groupData, FuncWork* work) {
    o39_colliders = HuMemAllocTag(COLLIDERS_MAX * sizeof(*o39_colliders), 31000);
    memset(o39_colliders, 0, COLLIDERS_MAX * sizeof(*o39_colliders));
    work->func = o39_UpdateColliders;
}
...
s32 o39_SetCollider(s16 modelId, Quad* baseQuad, f32 height, s16 maxTrackers, s32 attr) {
    ...
    for (i = 0; i < COLLIDERS_MAX; i++, collider++) {
        if (!collider->set) {
            ...
            if (maxTrackers != 0) {
                collider->modelTrackers = HuMemAllocTag(maxTrackers * sizeof(*collider->modelTrackers), 31000);
            }
...
ModelTracker* o39_SetModelTracker(s16 colliderId, s16 modelId) {
    ...
    if (!o39_colliders[colliderId].set) {
        return NULL;
    }
    if (o39_colliders[colliderId].trackerCount >= o39_colliders[colliderId].maxTrackers) {
        return NULL;
    }
    tracker = &o39_colliders[colliderId].modelTrackers[o39_colliders[colliderId].trackerCount];
    model = &HmfModelData[modelId];
    tracker->posX = &model->pos.x;
    tracker->posY = &model->pos.y;
    tracker->posZ = &model->pos.z;
    o39_colliders[colliderId].trackerCount++;
    return tracker;
}

void o39_SetColliderBaseQuad(s16 colliderId, Quad* baseQuad, f32 height) {
    if (colliderId < COLLIDERS_MAX && o39_colliders[colliderId].set) {
        QuadCollider* collider = &o39_colliders[colliderId];
...
void o39_UpdateColliders(FuncGroupData* groupData, FuncWork* work) {
    ...
    for (i = 0; i < COLLIDERS_MAX; i++, collider++) {
        ...
        o39_MakeTransformMtx(*collider->posX, *collider->posY, *collider->posZ, *collider->rotX, *collider->rotY, *collider->rotZ, collider->transMtx);
        o39_ApplyTransformMtx(collider->baseQuad.vertices, 4, collider->transMtx, currQuad);
        for (j = 0; j < collider->trackerCount; j++) {
            ...
            side = o39_CalcSideOfEdge(collider, model, &currQuad[o39_quadEdges[0][0]], &currQuad[o39_quadEdges[0][1]]);
            if (side == 0.0f) {
                model->colliding = FALSE;
                continue;
            }
            if (side > 0.0f) {
                ...
                for (k = 1; k < 4; k++) {
                    side = o39_CalcSideOfEdge(collider, model, &currQuad[o39_quadEdges[k][0]], &currQuad[o39_quadEdges[k][1]]);
                    if (side == 0.0f) {
                        model->colliding = TRUE;
                        break;
                    }
...
void o39_MakeTranslationMtx(f32* out, f32 x, f32 y, f32 z) {
    o39_MakeIdentityMtx(out);
    M_ID(out, 3, 0) = x;
    M_ID(out, 3, 1) = y;
    M_ID(out, 3, 2) = z;
}

void o39_MakeRotXMtx(f32* out, f32 angle) {
    o39_MakeIdentityMtx(out);
...
void o39_MakeRotYMtx(f32* out, f32 angle) {
    o39_MakeIdentityMtx(out);
...
void o39_MultiplyMtx(f32* a, f32* b, f32* out) {
    s16 i, j;
...
void o39_MakeTransformMtx(f32 transX, f32 transY, f32 transZ, f32 rotX, f32 rotY, f32 rotZ, f32* out) {
    ...
    o39_MakeTranslationMtx(translationMtx, transX, transY, transZ);
    o39_MakeRotXMtx(rotXMtx, rotX);
    o39_MakeRotYMtx(rotYMtx, rotY);
    o39_MultiplyMtx(rotXMtx, rotYMtx, rotMtx);
    o39_MultiplyMtx(rotMtx, translationMtx, out);
}
