...
void o39_UpdateColliders(FuncGroupContext* groupCtx, FuncContext* ctx);
...
FuncGroup* o39_CreateFuncGroup(omObjData* object, s8 groupId, s16 maxModels, s16 maxFuncs) {
    ...
    if (maxFuncs != 0) {
        group->funcCtx = HuMemAllocTag(maxFuncs * sizeof(*group->funcCtx), 31000);
        memset(group->funcCtx, 0, maxFuncs * sizeof(*group->funcCtx));
        group->order = HuMemAllocTag((maxFuncs + 1) * sizeof(*group->order), 31000);
    }
    group->maxFuncs = maxFuncs;
    group->groupCtx = HuMemAllocTag(sizeof(*group->groupCtx), 31000);
    if (maxModels != 0) {
        group->groupCtx->modelIds = HuMemAllocTag(maxModels * sizeof(*group->groupCtx->modelIds), 31000);
        memset(group->groupCtx->modelIds, 0, maxModels * sizeof(*group->groupCtx->modelIds));
    }
...
s16 o39_SetFunc(s8 groupId, s32 prio, void* data, void (*func)(FuncGroupContext*, FuncContext*), s8 tag) {
...
void o39_ResetFunc(s8 groupId, FuncContext* ctx) {
...
void o39_ResetFuncGroup(s8 groupId) {
    ...
    memset(group->funcCtx, 0, group->maxFuncs * sizeof(*group->funcCtx));
    group->sort = TRUE;
}
...
void o39_CreateColliders(FuncGroupContext* groupCtx, FuncContext* ctx) {
...
u8 o39_CalcLerp(f32* out, u8* timer, f32 start, f32 end, f32 duration) {
