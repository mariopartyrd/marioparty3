...
void M257_CreateMinigame(void) {
    ...
    M257_minigame = HuMemAllocTag(sizeof(*M257_minigame), 31000);
    memset(M257_minigame, 0, sizeof(*M257_minigame));
    M257_players = HuMemAllocTag(PLAYERS_TOTAL * sizeof(*M257_players), 31000);
    memset(M257_players, 0, PLAYERS_TOTAL * sizeof(*M257_players));
}
...
void M257_ExecClockIntro(FuncGroupContext* groupCtx, FuncContext* ctx) {
    ...
    switch (ctx->state) {
        case 0:
            ...
            ctx->work.work16[3] = 0;
            for (i = 0; i < PLAYERS_TOTAL; i++) {
                ctx->work.work16[i] = M257_SetFunc(FGRP_PLAYER_1 + i, 0xF, &M257_players[i], M257_ExecPlayerIntro, 0);
            }
            ctx->state = 1;
            break;
        case 1:
    ...
    if (ctx->work.work8[0] == 0) {
        ...
        if (M257_minigame->clockHandRot[HAND_OBSTACLE] > 360.0f) {
            ...
            for (i = 0; i < PLAYERS_TOTAL; i++) {
                M257_ResetFunc(FGRP_PLAYER_1 + i, &M257_funcGroups[FGRP_PLAYER_1 + i].funcCtx[ctx->work.work16[i]]);
            }
    ...
    if (M257_minigame->jumpsSinceSpeedChange > M257_minigame->jumpsForSpeedChange) {
        if (!ctx->work.work8[1]) {
            ...
            switch (rand16() % 3) {
                case 0:
                    if (rand16() & 1) {
                        ...
                        ctx->work.workF[1] = M257_GetRandInRange(3, 4);
                    } else {
                        ctx->work.workF[0] = M257_GetRandInRange(3, 4);
                ...
                case 1:
                    ...
                    break;
                case 2:
                    ctx->work.workF[0] = M257_GetRandInRange(2, 3);
            ...
            if (ABS(clockObstacleRot - clockPlatformRot) > ctx->work.workF[3]) {
                ...
                M257_minigame->clockBaseSpeed[HAND_OBSTACLE] = ctx->work.workF[0];
                M257_minigame->clockBaseSpeed[HAND_PLATFORM] = ctx->work.workF[1];
...
void M257_ExecPlayerKnockedOut(FuncGroupContext* groupCtx, FuncContext* ctx) {
    ...
    if (ctx->state == 0) {
        ...
        ctx->work.workF[0] = M257_minigame->clockHandRot[HAND_PLATFORM] + rotDirection * 90.0f;
