#include "common.h"
#include "game/object.h"
#include "game/board.h"
#include "FFB10.h"
#include "105D50.h"
#include "FA250.h"
#include "1006F0.h"
#include "EB2F0.h"
#include "ECA50.h"
#include "F9DE0.h"
#include "106640.h"
#include "F2910.h"
#include "F5070.h"
#include "F5E80.h"
#include "106A50.h"
#include "EE660.h"
#include "1063F0.h"
#include "10C230.h"
#include "EC3B0.h"
#include "101840.h"
#include "113520.h"
#include "113750.h"
#include "F5B90.h"
#include "10AD50.h"
#include "EDDB0.h"

enum DIFFICULTIES {
    EASY = 0,
    NORMAL = 1,
    HARD = 2,
    VERY_HARD = 3
};

extern u8 D_800CC0BA_main;
extern s16 D_800D1380_main;
extern s32 D_800A12CC_main;
extern s16 D_800CC4A0_main;
extern s16 D_800D2130_main;
extern s32 D_80105660_shared_board;
extern s16 D_800CDD64_main;
extern s16 kakusiMasuStarHisIdx;
extern s16 kakusiMasuCoinHisIdx;
extern s16 D_800D1708_main;
extern s16 kakusiMasuItemHisIdx;
extern u8 D_800CB8B1_main[];
extern s16 D_800CE206;
extern s32 D_80105668_shared_board;
extern s16 D_8010566C_shared_board;
extern s16 D_800D037E_main;
extern s32 mbTurnOnF;
extern s16 D_8010566E_shared_board;
extern Vec D_80105670_shared_board;
extern Vec *D_8010567C_shared_board;
extern Vec D_80105680_shared_board[];
extern Object *mbBattleKuriboMdl;
extern Object *mbBattleKuriboUpperMdl;
extern omObjData *mbBattleKuriboObj;
extern s16 D_800A12C8_main;
extern f32 D_800C9938_main;
extern s16 D_800CE206;
extern s32 mbWalkNum;
extern s16 D_800D1708_main;
extern s16 D_800D4080_main;
extern s16 kakusiMasuItemHisIdx;
extern s16 kakusiMasuStarHisIdx;
extern s16 kakusiMasuCoinHisIdx;
extern u16 D_800D03FC;
extern u16 D_800CE208;
extern u16 D_800CDD68_main;
extern s16 kakusiMasuItemHis[10];
extern s16 kakusiMasuCoinHis[10];
extern s16 kakusiMasuStarHis[10];
extern u16 D_80101DE0_shared_board[];
extern u16 D_80101E44_shared_board[];
extern s16 D_800CC698_main;
extern s16 D_80105664_shared_board;
extern s16 D_80105666_shared_board;

static void MBSaveInit(void);

s32 D_80101B40_shared_board[] = {
    ChillyWaters,
    w02,
    w03,
    w04,
    w05,
    w06,
    w10,
    ChillyWaters,
    selmenu2
};

void (*D_80101B64_shared_board)(void) = NULL;
void (*D_80101B68_shared_board)(void) = NULL;
void (*D_80101B6C_shared_board)(void) = NULL;

f32 D_80101B70_shared_board[][2] = {
    { -7.0f, -7.0f },
    { 7.0f, -7.0f },
    { 7.0f, 7.0f },
    { -7.0f, 7.0f }
};

f32 D_80101B90_shared_board[][2] = {
    { -7.0f, 4.0f },
    { 0.0f, -8.0f },
    { 7.0f, 4.0f },
};

f32 D_80101BA8_shared_board[][2] = {
    { -5.0f, 0.0f },
    { 5.0f, 0.0f },
};

f32 D_80101BB8_shared_board[][2] = {
    { -10.0f, -10.0f },
    { 10.0f, -10.0f },
    { 10.0f, 10.0f },
    { -10.0f, 10.0f },
};

f32 D_80101BD8_shared_board[][2] = {
    { -10.0f, 0.0f },
    { 0.0f, -10.0f },
    { 10.0f, 0.0f },
};

f32 D_80101BF0_shared_board[][2] = {
    { -7.0f, -7.0f },
    { 7.0f, -7.0f }
};

u8 D_80101C00_shared_board[] = {
    MINIGAME_ID(game_guys_roulette),
    MINIGAME_ID(game_guys_lucky_7),
    MINIGAME_ID(game_guys_magic_boxes),
    MINIGAME_ID(game_guys_sweet_surprise)
};

u8 D_80101C04_shared_board[] = {
    5,  // 5% game_guys_roulette
    30, // 25% game_guys_lucky_7
    65, // 35% game_guys_magic_boxes
    100 // 35% game_guys_sweet_surprise
};

// there's probably a better way to do this
#define GAME_GUY_RANDOM_MAX 1000

typedef struct CPU_GAMEGUY_STATS {
    u16 multiplier;
    u16 weight;
} CPU_GAMEGUY_STATS;

CPU_GAMEGUY_STATS D_80101C08_shared_board[] = {
    { 0, 500 },  // 0x multiplier - rolls 1-500 (50.0%)
    { 2, 870 },  // 2x multiplier - rolls 501-870 (37.0%)
    { 4, 950 },  // 4x multiplier - rolls 871-950 (8.0%)
    { 8, 985 },  // 8x multiplier - rolls 951-985 (3.5%)
    { 16, 995 }, // 16x multiplier - rolls 986-995 (1.0%)
    { 32, 999 }, // 32x multiplier - rolls 996-999 (0.4%)
    { 64, 1000 } // 64x multiplier - roll 1000 (0.1%)
};

// CPU enter shop probabilites per difficulty
u16 D_80101C24_shared_board[] = {
    [EASY] 75,
    [NORMAL] 85,
    [HARD] 90,
    [VERY_HARD] 95
};

u16 D_80101C2C_shared_board[][8] = {
    [SHOP_TOAD] {
        ITEM_MUSHROOM,
            ITEM_SKELETON_KEY,
            ITEM_CELLULAR_SHOPPER,
            ITEM_WARP_BLOCK,
            ITEM_DUEL_GLOVE,
            ITEM_GOLDEN_MUSHROOM,
            ITEM_BOO_REPELLENT,
            ITEM_MAGIC_LAMP
    },

    [SHOP_BABY_BOWSER] {
        ITEM_SKELETON_KEY,
            ITEM_POISON_MUSHROOM,
            ITEM_REVERSE_MUSHROOM,
            ITEM_PLUNDER_CHEST,
            ITEM_BOWSER_PHONE,
            ITEM_LUCKY_LAMP,
            ITEM_BOWSER_SUIT,
            ITEM_BOO_BELL
    },
};

// shop messages
s32 D_80101C4C_shared_board[][8] = {
    [SHOP_TOAD] {
        0x00003C23,
            0x00003C24,
            0x00003C27,
            0x00003C28,
            0x00003C2B,
            0x00003C2D,
            0x00003C2F,
            0x00003C31
    },
    [SHOP_BABY_BOWSER] {
        0x00003C24,
            0x00003C25,
            0x00003C26,
            0x00003C29,
            0x00003C2A,
            0x00003C2C,
            0x00003C2E,
            0x00003C30
    },
};

u16 D_80101C8C_shared_board[][5][8] = {
    {
        { 30, 50, 60, 70, 70, 90, 90, 100 },
        { 15, 30, 40, 50, 50, 85, 100, 100 },
        { 15, 30, 45, 60, 60, 100, 100, 100 },
        { 10, 22, 32, 32, 47, 67, 85, 100 },
        { 10, 20, 20, 20, 37, 57, 70, 100 },
    },
    {
        { 30, 50, 60, 70, 70, 70, 90, 100 },
        { 15, 30, 40, 50, 50, 80, 100, 100 },
        { 15, 30, 50, 60, 80, 90, 100, 100 },
        { 10, 10, 30, 40, 55, 75, 85, 100 },
        { 10, 10, 25, 40, 50, 70, 70, 100 },
    },
};

u16 D_80101D2C_shared_board[][4][3] = {
    {
        { 0x0000, 0x0001, 0x0002 },
        { 0x0000, 0x0000, 0x0003 },
        { 0x0000, 0x0000, 0x0004 },
        { 0x0000, 0x0000, 0x0004 },
    },
    {
        { 0x0000, 0x0001, 0x0002 },
        { 0x0000, 0x0000, 0x0003 },
        { 0x0000, 0x0000, 0x0004 },
        { 0x0000, 0x0000, 0x0004 },
    },
};

f32 D_80101D5C_shared_board[][2] = {
    { 0.0f, 0.0f },
    { 200.0f, 250.0f },
    { 110.0f, 250.0f },
    { 70.0f, 290.0f },
    { 0.0f, 360.0f },
};

f32 D_80101D84_shared_board[][2] = {
    { 0.0f, 0.0f },
    { 20.0f, 70.0f },
    { -70.0f, 70.0f },
    { 70.0f, 290.0f },
    { 0.0f, 360.0f },
};

s8 D_80101DAC_shared_board[][4][2] = {
    {
        { -1, 1 },
        { 3, 2 },
        { 3, 3 },
        { 3, 3 },
    },
    {
        { 0, 1 },
        { -1, 1 },
        { 1, 2 },
        { 1, 2 },
    },
    {
        { 0, 1 },
        { 0, 1 },
        { -1, 1 },
        { 1, 2 },
    },
    {
        { 0, 0 },
        { 0, 1 },
        { 0, 1 },
        { -1, -1 },
    },
};

s16 D_80101DCC_shared_board[][3] = {
    { 45, 90, 100 },
    { 70, 95, 100 },
    { 85, 95, 100 }
};

u16 D_80101DE0_shared_board[] = { 20, 30, 50 };

u8 D_80101DE8_shared_board[9][3] = {
    { 4, 7, 11 },   // 10 turns
    { 6, 11, 16 },  // 15 turns
    { 6, 16, 21 },  // 20 turns
    { 9, 17, 26 },  // 25 turns
    { 11, 21, 31 }, // 30 turns
    { 11, 21, 36 }, // 35 turns
    { 14, 27, 41 }, // 40 turns
    { 16, 31, 46 }, // 45 turns
    { 16, 36, 51 }, // 50 turns
};

// outcomes for how many coins are wagered for battle minigames
// If it's early game, a 30% chance is rolled. If successful, the battle minigame will be for 0 coins. Otherwise, run the table below
// order is probability of: 0 coins, 10 coins, 20 coins, 30 coins, 50 coins
// 1st place
u8 D_80101E04_shared_board[3][5] = {
    { 10, 95, 100, 0, 0 }, // early game
    { 10, 85, 100, 0, 0 }, // mid game
    { 10, 75, 100, 0, 0 }, // late game
};

// 2nd place
u8 D_80101E14_shared_board[3][5] = {
    { 5, 75, 95, 100, 0 }, // early game
    { 5, 65, 90, 100, 0 }, // mid game
    { 5, 55, 85, 100, 0 }, // late game
};

// 3rd place
u8 D_80101E24_shared_board[3][5] = {
    { 5, 65, 85, 95, 100 }, // early game
    { 5, 50, 80, 95, 100 }, // mid game
    { 5, 40, 70, 90, 100 }, // late game
};

// 4th place
u8 D_80101E34_shared_board[3][5] = {
    { 5, 50, 80, 95, 100 }, // early game
    { 5, 35, 70, 90, 100 }, // mid game
    { 5, 25, 60, 85, 100 }, // late game
};

// battle coin amounts (map index -> coin amount)
u16 D_80101E44_shared_board[] = {
    0, 10, 20, 30, 50
};

u8 (*D_80101E50_shared_board[])[3][5] = {
    &D_80101E04_shared_board,
    &D_80101E14_shared_board,
    &D_80101E24_shared_board,
    &D_80101E34_shared_board
};

Unk3 *D_80101E60_shared_board = NULL;
Unk3 *D_80101E64_shared_board = NULL;
Unk3 *D_80101E68_shared_board = NULL;
Unk3 *D_80101E6C_shared_board = NULL;

u8 D_80101E70_shared_board[][2] = {
    { ITEM_SKELETON_KEY, 72 },
    { ITEM_MAGIC_LAMP, 80 },
    { ITEM_KOOPA_KARD, 85 },
    { ITEM_WACKY_WATCH, 90 },
    { ITEM_LUCKY_CHARM, 95 },
    { ITEM_BARTER_BOX, 100 },
};

// unused? padding?
u8 D_80101E7C_shared_board[2] = { 0, 0 };

u8 D_80101E80_shared_board[] = {
    5, 10, 10, 20, 20, 30
};

void MBOvlCall(s32 id, s16 event, u16 stat) {
    omOvlHisData *overlay = &mbovlhis[mbovlhisidx++];

    if (id != -2) {
        if (id == -1) {
            id = omovlhis[omovlhisidx].overlayID;
            overlay->overlayID = id;
        } else {
            overlay->overlayID = id;
        }
    } else {
        id = D_80101B40_shared_board[GwSystem.current_board_index];
        overlay->overlayID = id;
    }

    overlay->event = event;
    overlay->stat = stat;

    if (mbovlhisidx >= ARRAY_COUNT(mbovlhis)) {
        mbovlhisidx = ARRAY_COUNT(mbovlhis) - 1;
    }
}

void MBOvlCallInit(void) {
    D_800CD2A0_main.unk_02 = 1;
    mbovlhisidx = 0;

    if (GWBoardFlagCheck(1) == 0) {
        if ((GwSystem.current_board_index != 6) && (MBOvlCall(-2, 4, 0x192), (GwSystem.current_board_index != 6))) {
            MBOvlCall(0x53, 0, 0x192);
        } else {
            MBOvlCall(0x54, 0, 0x192);
        }

        MBOvlCall(-2, 1, 0x192);
    }

    MBSaveInit();
    func_800ECF9C_shared_board(GwSystem.message_speed);
    _ClearFlag(0xC);
    D_800D6A48_main = 0;
}

void MBOvlCallExec(void) {
    omOvlHisData *overlay;

    D_800D4190_main.stat = 1;
    if (mbovlhisidx != 0) {
        mbovlhisidx--;
        overlay = &mbovlhis[mbovlhisidx];
        omOvlCallEx(overlay->overlayID, overlay->event, overlay->stat);
        return;
    }
    if (D_800CD2A0_main.unk_02 != 0) {
        D_800D4190_main.stat = 0;
        omOvlCallEx(D_80101B40_shared_board[GwSystem.current_board_index], 2, 0x192);
        return;
    }
    if (D_800D6A48_main != 0) {
        _SetFlag(0xC);
    } else {
        _ClearFlag(0xC);
    }
    omOvlReturnEx(1);
}

void func_800F884C_shared_board(void) {
    D_800CD2A0_main.unk_02 = 0;
}

s16 func_800F8858_shared_board(void) {
    return D_800D4190_main.stat;
}

static void func_800F8864_shared_board(s16 arg0) {
    s16 temp_v0 = mbovlhisidx;

    mbovlhis[mbovlhisidx].event = arg0;
    mbovlhisidx = temp_v0 + 1;
    if (mbovlhisidx >= ARRAY_COUNT(mbovlhis)) {
        mbovlhisidx = ARRAY_COUNT(mbovlhis) - 1;
    }
    MBOvlCall(-2, 3, 0x192);
    omOvlReturnEx(1);
}

void func_800F88D0_shared_board(void) {
    if (GWBoardFlagCheck(0x17)) {
        GWBoardFlagSet(0x10);
    }
}

s32 func_800F88FC_shared_board(void) {
    return mbGameMode;
}

void func_800F8908_shared_board(void) {
    s32 *temp_s0 = HuPrcCurrentGet()->user_data;

    while (1) {
        if (
            (MBPlayerComCheck(temp_s0[0]) == 0) &&
            (D_800A12D8_main == 0) &&
            ((GwSystem.current_player_index != temp_s0[0]) || (D_800A12D4_main == 0)) &&
            (D_800C9520_main[GwPlayer[temp_s0[0]].pad] & 0x20)) {
            func_8004ACE0_main(0x2BD, temp_s0[0]);
        }
        HuPrcVSleep();
    }
}

void MBObjectSetup(s32 arg0, s16 arg1, s32 arg2, s32 unused) {
    Process *temp_v0_2;
    UnkOvl81_1 *temp_v0_3;
    u8 playerIndex;
    Object *temp_v0;
    s32 i;
    GW_SYSTEM *system;

    func_800E9EF4_shared_board();
    func_800E6630_shared_board((u32)hvq_data_ROM_START);

    if (arg0 >= 0) {
        MBBackLoad(arg0);
    }

    func_800EA60C_shared_board();
    if (arg1 >= 0) {
        MBMasuCreate(0x13, (s32)arg1);
    }

    func_800D76D0_shared_board(arg2);
    MBModelInit();
    func_800F26E8_shared_board();
    MBKSuitInit();
    func_800F25D8_shared_board(0);
    func_800F25D8_shared_board(1);
    func_800F25D8_shared_board(2);
    func_800F25D8_shared_board(3);

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        MBModelTempAllocFree(MBPlayerGet(i)->player_obj);
        temp_v0 = MBPlayerGet(i)->player_obj;
        temp_v0->flags |= 2;
        MBModelDispOn(MBPlayerGet(i)->player_obj);
    }

    system = &GwSystem;
    playerIndex = system->current_player_index;
    if ((playerIndex < 4) && (GwPlayer[system->current_player_index].itemTurn != 0)) {
        MBKSuitCreate();
        MBModelDispOff(GwPlayer[system->current_player_index].player_obj);
    }

    func_8001FDE8_main(MBPlayerGet(0)->player_obj->omObj1->model[0]);
    func_8001FDE8_main(MBPlayerGet(1)->player_obj->omObj1->model[0]);
    func_8001FDE8_main(MBPlayerGet(2)->player_obj->omObj1->model[0]);
    func_8001FDE8_main(MBPlayerGet(3)->player_obj->omObj1->model[0]);
    func_800F2A20_shared_board();
    func_800DF854_shared_board();
    MBCoinChangeInit();
    MBTelopInit();
    func_800F453C_shared_board();
    func_800DB5DC_shared_board();
    func_8005A6B0_main();
    func_800ECAB4_shared_board();
    D_80105664_shared_board = -1;
    D_800D1360.unk_20 = 0;
    D_80105666_shared_board = 0;
    func_800F27D0_shared_board();
    D_80101B64_shared_board = NULL;
    D_80101B68_shared_board = NULL;
    D_80101B6C_shared_board = NULL;
    func_8004CF30_main();

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        temp_v0_2 = omAddPrcObj(func_800F8908_shared_board, 0, 0, 0x40);
        temp_v0_3 = HuMemMemoryAlloc(temp_v0_2->heap, sizeof(UnkOvl81_1));
        temp_v0_2->user_data = temp_v0_3;
        temp_v0_3->unk_00 = i;
        omPrcSetStatBit(temp_v0_2, 0xA0);
    }
}

void MBExit(void) {
    func_800DCB8C_shared_board();
    func_800F4730_shared_board();
    MBTelopClose();
    func_800E18FC_shared_board();
    MBMgCallClose();
    func_800F2A34_shared_board();
    func_8005F524_main();
    MBKSuitClose();
    func_800F27C4_shared_board();
    MBModelClose();
    func_800D7714_shared_board();
    func_800EB09C_shared_board();
    func_800EA694_shared_board();
    MBBackKill();
    MBBackClose();
    func_800E66D4_shared_board();
    func_800EA284_shared_board();
    if (D_80101B64_shared_board != NULL) {
        D_80101B64_shared_board();
    }
    if (D_80101B68_shared_board != NULL) {
        D_80101B68_shared_board();
    }
    func_800FC8A4_shared_board();
}

void MBHatenaFuncSet(void (*arg0)(void)) {
    D_80101B6C_shared_board = arg0;
}

void func_800F8D54_shared_board(void (*arg0)(void)) {
    D_80101B64_shared_board = arg0;
}

void MBKinokoEffKillHookSet(void (*arg0)()) {
    D_80101B68_shared_board = arg0;
}

void func_800F8D6C_shared_board(s32 arg0) {
    D_800CC0B8_main = arg0;
}

s16 func_800F8D78_shared_board(void) {
    return D_800CC0B8_main;
}

static void MBSaveInit(void) {
    GW_PLAYER *player;
    s32 i, j;
    GW_SYSTEM *system = &GwSystem;
    GW_COMMON *common = &GwCommon;

    mbGameMode = 0;
    mbWalkNum = 0;
    D_800CDD68_main = 0;
    D_800CE208 = 0;
    D_800D03FC = 0;
    kakusiMasuItem = -1;
    kakusiMasuStar = -1;
    kakusiMasuCoin = -1;
    D_800D1708_main = 0;
    D_800D4080_main = 0;
    D_800D2130_main = 0;

    for (i = 0; i < 10; i++) {
        kakusiMasuCoinHis[i] =
            kakusiMasuStarHis[i] =
                kakusiMasuItemHis[i] = -1;
    }

    kakusiMasuItemHisIdx = 0;
    kakusiMasuStarHisIdx = 0;
    kakusiMasuCoinHisIdx = 0;
    D_800C9938_main = 1.0f;

    MBMgCallHisInit();
    func_800DB56C_shared_board(); // clear lucky 7 flag for buffing golden mushrooms

    D_800A12C8_main = -1;
    D_800C9930_main = -1;
    D_800CE206 = 0;
    D_800A12D4_main = 1;
    D_800A12D8_main = 0;

    func_800F2484_shared_board(0);
    func_800F2484_shared_board(1);
    func_800F2484_shared_board(2);
    func_800F2484_shared_board(3);

    if (((common->unk9E == 3) | (common->unk9E == 1)) != 0) {
        common->unk9E = 2;
    }
    if (common->unk9F == 3) {
        common->unk9F = 0;
    }
    if (common->unkA0 == 3) {
        common->unkA0 = 1;
    }
    if (common->unkA1 == 3) {
        common->unkA1 = 1;
    }
    if (common->unkA2 == 3) {
        common->unkA2 = 0;
    }

    system->save_mode = common->unk9E;
    system->show_minigame_explanations = common->unk9F;
    system->message_speed = common->unkA0;
    system->walk_speed = common->unkA1;
    system->show_com_minigames = common->unkA2;

    if (GWBoardFlagCheck(1) == 0) {
        system->current_star_spawn = 0;
        system->current_turn = 1;
        system->current_player_index = 0;
        system->minigame_index = 0;

        for (i = 0; i < ARRAY_COUNT(GwPlayer); i++) { // clear all 5 "players" (bowser bomb remnants)
            player = MBPlayerGet(i);
            player->coin = 0;
            player->gamePrize = 0;
            player->coinPrize = 0;
            player->stats.prize.hatenaPrize = 0;
            player->stats.prize.redPrize = 0;
            player->stats.prize.bluePrize = 0;
            player->stats.prize.eventPrize = 0;
            player->stats.prize.gamblePrize = 0;
            player->stats.prize.kupaPrize = 0;
            player->stats.prize.battlePrize = 0;
            player->stats.prize.itemPrize = 0;
            player->stats.prize.bankPrize = 0;
            player->rev = 0;
            for (j = 0; j < ARRAY_COUNT(player->itemNo); j++) {
                player->itemNo[j] = -1;
            }
            player->itemTurn = 0;
        }

        // TODO: 16?
        for (i = 0; i < 16; i++) {
            GwSystem.boardData.halfWordBytes[i] = 0;
        }

        GWBoardFlagClear(6);
        GWBoardFlagClear(7);
        GWBoardFlagClear(8);
        GWBoardFlagClear(9);
        GWBoardFlagClear(10);
        GWBoardFlagClear(11);
        GWBoardFlagClear(12);
        GWBoardFlagClear(13);
        GWBoardFlagClear(18);
        GWBoardFlagClear(19);
        GWBoardFlagClear(20);
        GWBoardFlagClear(21);
        GWBoardFlagClear(22);
        GWBoardFlagClear(23);
        GWBoardFlagClear(2);
        GWBoardFlagClear(3);
        GWBoardFlagClear(4);
        GWBoardFlagClear(14);
        GWBoardFlagClear(1);
        GWBoardFlagClear(17);
        GWBoardFlagClear(16);

        if (_CheckFlag(0x1B) != 0) {
            GWBoardFlagSet(0xF);
        } else {
            GWBoardFlagClear(0xF);
        }

        if (GwSystem.playMode & 4) {
            if (_CheckFlag(0x28) != 0) {
                GWBoardFlagSet(0);
                return;
            }
            GWBoardFlagClear(0);
            return;
        }

        if (_CheckFlag(0x16) != 0) {
            GWBoardFlagSet(0);
            return;
        }

        GWBoardFlagClear(0);
    }
}

void MBCameraFocusModeSet(s16 arg0) {
    D_8010566E_shared_board = arg0;
}

s16 MBCameraFocusModeGet(void) {
    return D_8010566E_shared_board;
}

void MBCameraFocusStaticPosSet(Vec *arg0) {
    D_80105670_shared_board = *arg0;
}

void MBCameraFocusVecSet(Vec *arg0) {
    D_8010567C_shared_board = arg0;
}

static void func_800F91A4_shared_board(void) {
    Vec2f sp10;

    while (1) {
        switch (D_8010566E_shared_board) {
            case 1:
                MBCameraPos3DSet(&GwPlayer[GwSystem.current_player_index].player_obj->coords);
                break;
            case 2:
                MBCameraPos3DSet(&D_80105670_shared_board);
                break;
            case 4:
                MBCameraPos3DSet(D_8010567C_shared_board);
                break;
            case 3:
                MBCameraPos2DGet(&sp10);
                MBCameraPos2DSet(&sp10);
                break;
        }
        HuPrcVSleep();
    }
}

static void MBNextPlayerTurn(void) {
    GW_SYSTEM *system = &GwSystem;

    system->current_player_index++;

    if (system->current_player_index >= MB_MAX_PLAYERS) {
        system->current_player_index = 0;
        if (mbTurnOnF == 0) {
            if (GwSystem.wackyWatchUsedState != WATCH_USED) {
                system->current_turn++;
            } else {
                GwSystem.wackyWatchUsedState = WATCH_SPENT;
                system->current_turn = system->total_turns - 4;
            }
            if (system->current_turn >= 99) {
                system->current_turn = 99;
            }
        }
    }
}

static s32 MBLast5Check(void) {
    GW_SYSTEM *system = &GwSystem;

    // TODO: is there another way to write this?
    if ((system->current_player_index == MB_MAX_PLAYERS - 1) || (system->current_player_index < 0) || (system->current_player_index >= MB_MAX_PLAYERS)) {
        if (system->total_turns >= 0) {
            if (system->current_turn == (system->total_turns - 5)) {
                return 1;
            }
        }

        if (GwSystem.wackyWatchUsedState == WATCH_USED) {
            return 1;
        }
    }

    return 0;
}

static void func_800F93A4_shared_board(void) {
    s32 i;

    do {
        HuPrcVSleep();
    } while (WipeStatGet() != 0);

    while (1) {
        HuPrcVSleep();
        if (D_800D530C_main == 0 && D_80105664_shared_board < 0) {
            continue;
        }

        while (WipeStatGet() != 0) {
            HuPrcVSleep();
        }

        if (D_80105666_shared_board != 0) {
            HuAudSeqFadeOut(0x5A);
            if (D_800C9930_main != -1) {
                func_800039A4_main(D_800C9930_main, 0x5A);
                D_800C9930_main = -1;
                func_8004A950_main();
            }
            if (D_800CE206 != 0) {
                func_8004A950_main();
                D_800CE206 = 0;
            }
        }

        WipeCreateOut(D_800D1360.unk_20, 0x10);
        HuPrcSleep(0x11);
        MBExit();

        if (D_80105664_shared_board & 0x20) {
            D_800D6A48_main = 1;
            if (GwSystem.current_board_index == WALUIGIS_ISLAND) {
                _CheckFlag(9);
            }
            MBOvlCall(0x4F, 0, 0x4190);
            func_800F884C_shared_board();
        }

        if (D_80105664_shared_board & 1) {
            if (MBLast5Check() != 0) {
                MBOvlCall(0x51, 0, 0x192);
            }
            MBNextPlayerTurn();
        }

        if (D_80105664_shared_board & 2) {
            for (i = 0; i < MB_MAX_PLAYERS; i++) {
                if (mbStatusData[i].spaceType == SPACE_BLUE) {
                    D_800CB8B1_main[i] = 0;
                } else {
                    D_800CB8B1_main[i] = 1;
                }
            }
            MBOvlCall(0x70, 0, 0x192);
        }

        if (D_80105664_shared_board & 8) {
            MBOvlCall(D_80105668_shared_board, D_8010566C_shared_board, 0x192);
        }

        GWBoardFlagClear(5);
        omOvlReturnEx(1);
        omOvlKill();
        HuPrcVSleep();
    }
}

static void func_800F95FC_shared_board(s16 arg0) {
    switch (--arg0) {
        case 13:
            if (GwSystem.current_player_index != MB_MAX_PLAYERS) {
                HuAudFXPlay(0x123);
                return;
            }
        case 0:
        case 1:
        case 3:
        case 4:
        case 5:
        case 6:
        case 8:
        case 11:
        case 14:
            HuAudFXPlay(0x101);
            break;
    }
}

static void func_800F965C_shared_board(s16 arg0) {
    switch (--arg0) {
        case 3:
        case 5:
            HuAudFXPlay(0x108);
            break;
        case 0:
            HuAudFXPlay(0x121);
            break;
        case 1:
            HuAudFXPlay(0x122);
            break;
        case 11:
            HuAudFXPlay(0x124);
            break;
        case 4:
            HuAudFXPlay(0x12F);
            break;
        case 8:
        case 14:
            HuAudFXPlay(0x12E);
            break;
        case 6:
            HuAudFXPlay(0x130);
            break;
        case 13:
            HuAudFXPlay(0x123);
            break;
    }
}

void MBPlayerPosFixSet(s32 playerNo, s32 arg1) {
    GW_PLAYER *player;
    GW_PLAYER *playerCur;
    SpaceData *space;
    s32 var_s1;
    s32 var_s4;
    s32 playersShareSpace;
    s32 i;
    GW_SYSTEM *system = &GwSystem;

    player = MBPlayerGet(playerNo);
    space = MBMasuGet(MBMasuLinkMasuIdGet(player->clink, player->cidx));
    if (system->current_player_index == playerNo) {
        HuVecCopy3F(&player->player_obj->coords, &space->coords);
        return;
    }

    playersShareSpace = 0;
    playerCur = MBPlayerGet(system->current_player_index);
    var_s4 = 0;

    if (player->clink == playerCur->clink) {
        playersShareSpace = player->cidx == playerCur->cidx;
    }

    var_s1 = 0;
    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if ((playerNo != i) && (i != system->current_player_index)) {
            GW_PLAYER *curPlayer = MBPlayerGet(i);
            if ((player->clink == curPlayer->clink) && (player->cidx == curPlayer->cidx)) {
                var_s1 += 1;
                var_s4 += i < playerNo;
            }
        }
    }

    if (playersShareSpace == 1) {
        if (var_s1 == 2) {
            HuVecCopyXYZ(&D_80105680_shared_board[playerNo], space->coords.x + D_80101BD8_shared_board[var_s4][0], space->coords.y, space->coords.z + D_80101BD8_shared_board[var_s4][1]);
        } else if (var_s1 == 3) {
            HuVecCopyXYZ(&D_80105680_shared_board[playerNo], space->coords.x + D_80101BB8_shared_board[var_s4][0], space->coords.y, space->coords.z + D_80101BB8_shared_board[var_s4][1]);
        } else {
            HuVecCopyXYZ(&D_80105680_shared_board[playerNo], space->coords.x + D_80101BF0_shared_board[var_s4][0], space->coords.y, space->coords.z + D_80101BF0_shared_board[var_s4][1]);
        }
    } else {
        if (var_s1 == 0) {
            HuVecCopy3F(&D_80105680_shared_board[playerNo], &space->coords);
        } else if (var_s1 == 2) {
            HuVecCopyXYZ(&D_80105680_shared_board[playerNo], space->coords.x + D_80101B90_shared_board[var_s4][0], space->coords.y, space->coords.z + D_80101B90_shared_board[var_s4][1]);
        } else if (var_s1 == 3) {
            HuVecCopyXYZ(&D_80105680_shared_board[playerNo], space->coords.x + D_80101B70_shared_board[var_s4][0], space->coords.y, space->coords.z + D_80101B70_shared_board[var_s4][1]);
        } else {
            HuVecCopyXYZ(&D_80105680_shared_board[playerNo], space->coords.x + D_80101BA8_shared_board[var_s4][0], space->coords.y, space->coords.z + D_80101BA8_shared_board[var_s4][1]);
        }
    }

    if (arg1 < 2) {
        HuVecCopy3F(&player->player_obj->coords, &D_80105680_shared_board[playerNo]);
        return;
    }
    MBPlayerPosMoveCreate(&player->player_obj->coords, &D_80105680_shared_board[playerNo], &player->player_obj->coords, arg1);
}

// if player should jump to next space
s32 func_800F9A68_shared_board(s32 arg0) {
    Vec sp10;
    GW_PLAYER *curPlayer;
    GW_PLAYER *player;
    f32 var_f4;
    SpaceData *space;
    SpaceData *nextSpace;
    s32 var_s2;
    s32 var_s4;
    s32 i;
    Vec *temp;

    curPlayer = MBPlayerGet(arg0);
    var_s2 = 0;
    var_s4 = 0;

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if (i == arg0) {
            continue;
        }
        player = MBPlayerGet(i);
        if (curPlayer->clink == player->clink) {
            var_s4 += curPlayer->cidx == player->cidx;
        }
        if (curPlayer->nlink == player->clink) {
            var_s2 += curPlayer->nidx == player->cidx;
        }
    }

    space = MBMasuGet(MBMasuLinkMasuIdGet(curPlayer->clink, curPlayer->cidx));
    HuVecSubtract(&sp10, &MBMasuGet(MBMasuLinkMasuIdGet(curPlayer->nlink, curPlayer->nidx))->coords, &space->coords);
    var_f4 = MBVecAngleGet(&sp10);
    if ((var_s4 == 0) || (!(D_80101D5C_shared_board[var_s4][0] < var_f4)) || (!(var_f4 <= D_80101D5C_shared_board[var_s4][1]))) {
        if (var_s2 != 0) {
            if (D_80101D84_shared_board[var_s2][0] < 0.0f) {
                var_f4 -= 360.0f;
            }
            if (!(D_80101D84_shared_board[var_s2][0] < var_f4) || (!(var_f4 <= D_80101D84_shared_board[var_s2][1]))) {
                return 0;
            } else {
                return 1;
            }
        } else {
            return 0;
        }
    } else {
        return 1;
    }
}

static void func_800F9C68_shared_board(void) {
    HuPrcSleep(30);
    func_800035E8_main(D_800D037E_main);
    D_800CE206 = 0;
    omDelPrcObj(NULL);
}

static void MBBattleKuriboObjMain(omObjData *arg0) {
    mbBattleKuriboMdl->coords.y = HuMathSin(arg0->rot.x) * 3.0f;
    mbBattleKuriboUpperMdl->coords.y = HuMathSin(arg0->rot.x) * 3.0f;
    arg0->rot.x += 10.0f;
}

// related to how CPUs decide who to duel?
static s32 func_800F9D1C_shared_board(void) {
    s32 i;
    s32 var_s1;
    s32 var_s2;
    s32 var_s3;
    s32 sp10[MB_MAX_PLAYERS];
    GW_SYSTEM *system = &GwSystem;

    for (i = 0, var_s2 = 99; i < MB_MAX_PLAYERS; i++) {
        if ((i != system->current_player_index) && ((GwSystem.unk_58 >> i) & 1) && (BoardPlayerRankCalc(i) < var_s2) && GwPlayer[i].coin != 0) {
            var_s2 = BoardPlayerRankCalc(i);
        }
    }

    var_s1 = 0;

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if ((i != system->current_player_index) && ((GwSystem.unk_58 >> i) & 1) && (BoardPlayerRankCalc(i) == var_s2)) {
            sp10[var_s1] = i;
            var_s1 += 1;
        }
    }

    if (MBPlayerGet(CUR_PLAYER)->coin >= 20) {
        if (MBPlayerGet(sp10[0])->coin < 20) {
            for (i = 0, var_s2 = 99, var_s3 = 0; i < MB_MAX_PLAYERS; i++) {
                if ((i != system->current_player_index) && (MBPlayerGet(i)->coin >= 20)) {
                    var_s3 = 1;
                    if (BoardPlayerRankCalc(i) < var_s2) {
                        var_s2 = BoardPlayerRankCalc(i);
                    }
                }
            }

            if (var_s3 != 0) {
                for (i = 0, var_s1 = 0; i < MB_MAX_PLAYERS; i++) {
                    if ((i != system->current_player_index) && ((GwSystem.unk_58 >> i) & 1) && (BoardPlayerRankCalc(i) == var_s2) && GwPlayer[i].coin != 0) {
                        sp10[var_s1] = i;
                        var_s1++;
                    }
                }
            }
        }
    }
    i = sp10[MBRand(var_s1)];
    if (GwPlayer[i].coin == 0) {
        i = 4;
    }
    return i;
}

static s32 func_800F9F84_shared_board(s32 playerNo) {
    s32 randRange;
    s32 minCoinAmount;
    s32 starDifference;
    s32 rank;
    GW_SYSTEM *system = &GwSystem;

    starDifference = GwPlayer[system->current_player_index].star - GwPlayer[playerNo].star;

    if ((GwPlayer[system->current_player_index].star - GwPlayer[playerNo].star) < 0) {
        starDifference = GwPlayer[playerNo].star - GwPlayer[system->current_player_index].star;
    }

    //@bug this should've been  if (starDifference >= 1) {
    if (starDifference <= 1) {
        starDifference = 1;
    }

    rank = BoardPlayerRankCalc(system->current_player_index);
    switch (D_80101DAC_shared_board[rank][playerNo][starDifference]) {
        case 0:
            randRange = 10;
            minCoinAmount = 90;
            break;
        case 1:
            randRange = 20;
            minCoinAmount = 60;
            break;
        case 2:
            randRange = 20;
            minCoinAmount = 20;
            break;
        default:
            randRange = 9;
            minCoinAmount = 1;
            break;
    }
    randRange = (minCoinAmount + MBRand(randRange)) + 1;
    return GwPlayer[system->current_player_index].coin * (randRange / 100.0f);
}

static s32 func_800FA104_shared_board(u8 *arg0) {
    s32 var_s2 = 0;
    s16 sp18[4];
    u8 sp20[4] = { 1, 2, 4, 8 };
    s32 j;
    s32 i;
    GW_SYSTEM *system = &GwSystem;

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if (i == system->current_player_index) {
            if (GwPlayer[i].stat & 1) {
                var_s2 = 1; // why does this change codegen
                var_s2 = func_800F9D1C_shared_board() + 1;
                sp18[GwPlayer[i].pad] = 0x400;
            } else {
                func_8005FE54_main(mbDlgWinId, sp20[GwPlayer[i].pad]);
                sp18[GwPlayer[i].pad] = -1;
            }
        } else {
            sp18[GwPlayer[i].pad] = 0;
        }
    }

    if (var_s2 != 0) {
        func_8005B63C_main(mbDlgWinId, 2, 2);
        for (j = 0; var_s2 != 0; j++) {
            var_s2--;
            if (var_s2 == 0) {
                sp18[GwPlayer[system->current_player_index].pad] = -0x8000;
            }
            if (j == 0) {
                func_8005F698_main(sp18[0], sp18[1], sp18[2], sp18[3], func_800EDC40_shared_board());
            } else {
                func_8005F698_main(sp18[0], sp18[1], sp18[2], sp18[3], 5);
            }
        }
    } else {
        func_8005F744_main(sp18[0], sp18[1], sp18[2], sp18[3]);
    }

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if (arg0[i] == 0) {
            func_8006010C_main(mbDlgWinId, i);
        }
    }

    i = 0;
    do {
        i = func_8005E1D8_main(mbDlgWinId, i, 1);
    } while (arg0[i] == 0);
    return i;
}

static void MBBattleKuriboEnd(void) {
    omDelObj(mbBattleKuriboObj);

    while (mbBattleKuriboMdl->velocity.x <= 120.0f) {
        HuPrcVSleep();
        mbBattleKuriboMdl->velocity.x += 3.0f;
        mbBattleKuriboUpperMdl->velocity.x += 3.0f;
    }

    MBModelKill(mbBattleKuriboMdl);
    MBModelKill(mbBattleKuriboUpperMdl);
}

void MBKettouResultExec(void) {
    char sp20[4];
    s32 temp_s1;
    s32 var_s0;
    GW_SYSTEM *system = &GwSystem;

    if ((GwPlayer[system->current_player_index].bonusCoin == 0) && (GwPlayer[GwSystem.unk_58].bonusCoin == 0)) {
        MBDlgWinExecY(6, 0x4006, 0x88);
        MBCoinChangeStatusCreate(system->current_player_index, GwSystem.playerIndexVisitingBowser);
        MBCoinTakeCreateSound(system->current_player_index, GwSystem.playerIndexVisitingBowser, 0);
        MBCoinChangeStatusCreate(GwSystem.unk_58, GwSystem.playerIndexVisitingBowser);
        MBCoinTakeCreateSound(GwSystem.unk_58, GwSystem.playerIndexVisitingBowser, 1);
    } else {
        var_s0 = (GwPlayer[system->current_player_index].bonusCoin != 0) ? system->current_player_index : GwSystem.unk_58;
        temp_s1 = GwSystem.playerIndexVisitingBowser * 2;
        sprintf(sp20, "%d", temp_s1);
        MBDlgWinInsertCreateY(6, 0x4005, mbCharNameMesTbl[GwPlayer[var_s0].chr], (u32)sp20, 0, 0, 0, 0x88);
        MBDlgWinClose();
        MBDlgWinKill();
        if (GwPlayer[system->current_player_index].bonusCoin != 0) {
            MBCoinChangeStatusCreate(system->current_player_index, temp_s1);
            MBCoinTakeCreateSound(system->current_player_index, temp_s1, 1);
        }
        if (GwPlayer[GwSystem.unk_58].bonusCoin != 0) {
            MBCoinChangeStatusCreate(GwSystem.unk_58, temp_s1);
            MBCoinTakeCreateSound(GwSystem.unk_58, temp_s1, 1);
        }
    }

    MBBattleKuriboEnd();
}

void MBBattleKuriboStart(GW_PLAYER *arg0, s32 arg1) {
    mbBattleKuriboMdl = MBModelCreate(0xC, NULL);
    Hu3DModelScaleSet(mbBattleKuriboMdl->omObj1->model[0], 0.0f, 0.0f, 0.0f);
    func_8001FDE8_main(mbBattleKuriboMdl->omObj1->model[0]);

    mbBattleKuriboMdl->flags |= 1;
    mbBattleKuriboUpperMdl = MBModelCreate(0x1F, NULL);

    Hu3DModelScaleSet(mbBattleKuriboUpperMdl->omObj1->model[0], 0.0f, 0.0f, 0.0f);
    HuVecCopy3F(&mbBattleKuriboMdl->coords, &arg0->player_obj->coords);
    HuVecCopy3F(&mbBattleKuriboUpperMdl->coords, &arg0->player_obj->coords);

    mbBattleKuriboMdl->velocity.x = 120.0f;
    mbBattleKuriboUpperMdl->velocity.x = 122.0f;

    while (mbBattleKuriboMdl->velocity.x > 30.0f) {
        if (arg1 == 0) {
            HuPrcVSleep();
        }
        mbBattleKuriboMdl->velocity.x -= 1.0f;
        mbBattleKuriboUpperMdl->velocity.x -= 1.0f;
    }
    mbBattleKuriboObj = omAddObj(0x1F4, 0, 0, -1, MBBattleKuriboObjMain);
    mbBattleKuriboObj->rot.x = 180.0f;
}

s32 MBKettouPlayerBitGet(s32 arg0) {
    GW_SYSTEM *system = &GwSystem;
    GW_PLAYER *player;
    s16 temp_s3;
    s16 var_s2;
    s32 i;

    var_s2 = 0;
    if ((system->total_turns - system->current_turn) >= 5) {
        return 0;
    }

    temp_s3 = MBMasuLinkMasuIdGet(MBPlayerGet(CUR_PLAYER)->clink, MBPlayerGet(CUR_PLAYER)->cidx);

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if (i != MBPlayerGet(CUR_PLAYER)->turn) {
            if (temp_s3 == MBMasuLinkMasuIdGet(MBPlayerGet(i)->clink, MBPlayerGet(i)->cidx)) {
                var_s2 |= 1 << i;
                if (arg0 != 0) {
                    MBPlayerVibrate(i, 3);
                }
            }
        }
    }
    return var_s2;
}

void MBKettouComResultSet(void) {
    s32 var_s0;
    s32 temp_a0_2;
    s32 var_a0;
    s32 var_a1;
    s32 temp_a3;
    s32 i;
    GW_SYSTEM *system = &GwSystem;
    s32 var_s1;

    WipeCreateOut(8, 0x10);
    HuPrcSleep(17);
    HuPrcSleep(5);
    var_s1 = GwSystem.unk_58;
    var_s0 = GwPlayer[system->current_player_index].cpu_difficulty - GwPlayer[var_s1].cpu_difficulty;

    if ((GwPlayer[system->current_player_index].cpu_difficulty - GwPlayer[var_s1].cpu_difficulty) < 0) {
        var_s0 = GwPlayer[GwSystem.unk_58].cpu_difficulty - GwPlayer[system->current_player_index].cpu_difficulty;
    }

    // if cpu difficulty difference was greater than 2, clamp to 2
    if (var_s0 > 2) {
        var_s0 = 2;
    }

    temp_a0_2 = MBRand(100.0f);

    for (i = 0; i < 3; i++) {
        if (!(D_80101DCC_shared_board[var_s0][i] < temp_a0_2)) {
            break;
        }
    }

    var_a0 = system->current_player_index;
    var_a1 = var_s1;
    temp_a3 = var_a0;

    if (GwPlayer[var_a0].cpu_difficulty < GwPlayer[var_a1].cpu_difficulty) {
        var_a0 = var_a1;
        var_a1 = temp_a3;
    }

    GwPlayer[var_a0].bonusCoin = 0;
    GwPlayer[var_a1].bonusCoin = 0;

    switch (i) {
        case 0:
            GwPlayer[var_a0].bonusCoin = 10;
            break;
        case 1:
            GwPlayer[var_a1].bonusCoin = 10;
            break;
    }

    mbItemBtnF = 0;
    func_8004A7C4_main(0x1E);
    HuPrcSleep(0xF);
    func_8004A72C_main(0x1E);
    mbItemBtnF = 1;
    D_800C9930_main = -1;
    D_800CE206 = 0;
    omPrcSetStatBit(omAddPrcObj(func_800F9C68_shared_board, 0, 0, 0), 0xA0);
    WipeCreateIn(8, 0x10);
    HuPrcSleep(17);
}

s32 MBKettouExec(GW_PLAYER *arg0, s32 arg1) {
    char sp20[MB_MAX_PLAYERS][16];
    u8 sp60[MB_MAX_PLAYERS + 2]; //+2?
    s16 sp68[2];
    s32 sp84;

    Process *temp_s0_2;
    s32 temp_v1;
    s32 var_s1_3;
    s32 var_s2;
    s32 var_s5;
    s32 var_a1;
    s32 var_s0_3;
    s32 var_s1;
    s32 var_s3;
    s32 var_s4;
    s32 var_s6;
    s32 var_s7;
    s32 i;
    s8 *showComMinigames = &GwSystem.show_com_minigames;

    var_s7 = 0;
    var_s4 = 0;
    mbItemBtnF = 0;
    func_8004A670_main(0xF);
    HuPrcSleep(0x10);
    D_800C9930_main = func_8004A5C4_main(0x3E);
    mbItemBtnF = 1;
    mbBattleKuriboMdl = MBModelCreate(0xC, NULL);
    Hu3DModelScaleSet(mbBattleKuriboMdl->omObj1->model[0], 0.0f, 0.0f, 0.0f);
    func_8001FDE8_main(mbBattleKuriboMdl->omObj1->model[0]);
    mbBattleKuriboMdl->flags |= 1;
    mbBattleKuriboUpperMdl = MBModelCreate(0x1F, NULL);
    Hu3DModelScaleSet(mbBattleKuriboUpperMdl->omObj1->model[0], 0.0f, 0.0f, 0.0f);
    HuVecCopy3F(&mbBattleKuriboMdl->coords, &arg0->player_obj->coords);
    HuVecCopy3F(&mbBattleKuriboUpperMdl->coords, &arg0->player_obj->coords);
    mbBattleKuriboMdl->velocity.x = 120.0f;
    mbBattleKuriboUpperMdl->velocity.x = 122.0f;

    while (mbBattleKuriboMdl->velocity.x > 30.0f) {
        HuPrcVSleep();
        mbBattleKuriboMdl->velocity.x -= 1.0f;
        mbBattleKuriboUpperMdl->velocity.x -= 1.0f;
    }

    mbBattleKuriboObj = omAddObj(0x1F4, 0, 0, -1, MBBattleKuriboObjMain);
    mbBattleKuriboObj->rot.x = 180.0f;
    HuAudFXPlay(0x25B);
    var_s1 = 0;

    if (GwPlayer[arg0->turn].coin != 0) {
        var_s6 = 0;

        do { // reg alloc hack
            // Count valid duel targets
            for (i = 0; i < MB_MAX_PLAYERS; i++) {
                if ((GwSystem.unk_58 >> i) & 1 && GwPlayer[i].coin != 0) {
                    var_s7++; // valid duel target
                    var_s4 = i;
                    var_s1++;
                    sp20[var_s4][0] = 8;
                    sp60[var_s4] = 1;
                    var_s6 |= 1 << var_s4;
                } else {
                    sp20[i][0] = 1; // invalid duel target (disable prompt?)
                    sp60[i] = 0;
                }

                func_800E2260_shared_board(i, &sp20[i][1]);
            }
        } while (0);

        if (var_s1 == 0) {
            MBDlgWinExecY(6, 0x4002, 0x88);
            MBBattleKuriboEnd();
            mbItemBtnF = 0;
            func_8004A7C4_main(0x1E);
            HuPrcSleep(0xF);
            func_8004A72C_main(0x1E);
            mbItemBtnF = 1;
            D_800C9930_main = -1;
            D_800CE206 = 0;
            omPrcSetStatBit(omAddPrcObj(func_800F9C68_shared_board, 0, 0, 0), 0xA0);
            HuPrcSleep(0x1F);
            return 0;
        } else {
            MBDlgWinExecY(6, 0x4000, 0x88);
        }
    } else {
        MBDlgWinExecY(6, 0x4001, 0x88);
        MBBattleKuriboEnd();
        mbItemBtnF = 0;
        func_8004A7C4_main(0x1E);
        HuPrcSleep(0xF);
        func_8004A72C_main(0x1E);
        mbItemBtnF = 1;
        D_800C9930_main = -1;
        D_800CE206 = 0;
        omPrcSetStatBit(omAddPrcObj(func_800F9C68_shared_board, 0, 0, 0), 0xA0);
        HuPrcSleep(0x1F);
        return 0;
    }

    // Target selection loop
    while (1) {
        if (var_s7 >= 2 || (GwSystem.playerIndexVisitingBowser & 0x8000)) {
        tempLabel:
            //?
            sp60[4] = 1;
            sp60[5] = 1;
            MBDlgWinInsertCreate(6, 0x5C02, (u32)sp20[0], (u32)sp20[1], (u32)sp20[2], (u32)sp20[3], 0);
            var_s4 = func_800FA104_shared_board(sp60);
            MBDlgWinClose();
            MBDlgWinKill();

            switch (var_s4) {
                case 4:
                    i = 0;
                    var_s1 = var_s7;
                    for (i = 0; i < MB_MAX_PLAYERS; i++) {
                        if (var_s6 & (1 << i)) {
                            if (var_s1-- != 0) {
                                break;
                            }
                        }
                    }
                    var_s4 = i;
                    break;
                case 5:
                    mbCameraBtnF = 1;
                    mbItemBtnF = 0;
                    omPrcSetStatBit(HuPrcCurrentGet(), 0x80);
                    MBMapScrollExec(GwPlayer[GwSystem.current_player_index].pad);
                    omPrcResetStatBit(HuPrcCurrentGet(), 0x80);
                    mbCameraBtnF = 0;
                    mbItemBtnF = 1;
                    func_800FC8A4_shared_board();
                    goto tempLabel; // Go back to selection menu
            }
        }

        var_s3 = 0;
        break; // Exit selection loop
    }

    // Coin wager selection
    sprintf(sp20[0], "%d", 999);
    func_8006022C_main((u32)sp20[0], 0);
    func_80060394_main(1, sp68, 0x4003);
    sp84 = MBWinCreate(6, sp68, 0x64);

    var_s1_3 = 1;

    var_s5 = 1;

    if (GwPlayer[GwSystem.current_player_index].coin <= GwPlayer[var_s4].coin) {
        var_s2 = GwPlayer[GwSystem.current_player_index].coin;
    } else {
        var_s2 = GwPlayer[var_s4].coin;
    }

    sprintf(sp20[0], "%d", var_s1_3);
    func_8005B6BC_main(sp84, (u32)&sp20, 0);
    func_8005B43C_main(sp84, 0x4003, -1, -1);
    func_80061388_main(sp84);

    if (MBPlayerComCheck(-1) != 0) {
        var_s3 = func_800F9F84_shared_board(var_s4);
        do { // reg alloc hack
            if (var_s2 < var_s3) {
                var_s3 = var_s2;
            }
            if (var_s3 <= 0) {
                var_s3 = 1;
            }
        } while (0);
    }

    var_s0_3 = 5;

    // Coin amount input loop
    while (1) {
        if (MBPlayerComCheck(-1) == 0) {
            // Player controlled input
            if (D_800C9520_main[GwPlayer[GwSystem.current_player_index].pad] & 0x8000) { //?
                break;                                                                    // Player confirmed selection
            }

            if (D_800D0590_main[GwPlayer[GwSystem.current_player_index].pad] & 0x800) {
                var_s1_3++;
            }
            if (D_800D0590_main[GwPlayer[GwSystem.current_player_index].pad] & 0x400) {
                var_s1_3--;
            }
            if (D_800D0590_main[GwPlayer[GwSystem.current_player_index].pad] & 0x200) {
                var_s1_3 -= 0xA;
            }
            if (D_800D0590_main[GwPlayer[GwSystem.current_player_index].pad] & 0x100) {
                var_s1_3 += 0xA;
            }
        } else {
            var_s0_3--;
            // AI controlled input
            if (var_s0_3 == -1) {
                if (var_s3 != var_s1_3) {
                    if ((var_s3 % 10) != (var_s1_3 % 10)) {
                        var_s1_3 += 1;
                        var_s0_3 = 5;
                    } else if ((var_s3 / 10) != (var_s1_3 / 10)) {
                        var_s1_3 += 0xA;
                        var_s0_3 = 5;
                    }
                } else {
                    break; // AI finished selecting amount
                }
            }
        }

        // Clamp value to valid range
        if (var_s5 == 1 && var_s1_3 < 1) {
            var_s1_3 = var_s2;
        }
        if (var_s5 == var_s2 && var_s2 < var_s1_3) {
            var_s1_3 = 1;
        }
        if (var_s1_3 <= 0) {
            var_s1_3 = 1;
        }
        if (var_s2 < var_s1_3) {
            var_s1_3 = var_s2;
        }

        if (var_s5 != var_s1_3) {
            HuAudFXPlay(0x246);
        }

        var_s5 = var_s1_3;

        sprintf(sp20[0], "%d", var_s1_3);
        func_8005B6BC_main(sp84, (u32)&sp20, 0);
        func_8005B43C_main(sp84, 0x4003, -1, -1);

        HuPrcVSleep();
    }

    HuAudFXPlay(0x247);
    func_80061A5C_main(sp84, 0);
    func_8005F364_main(sp84);
    HuPrcSleep(0xA);
    MBCoinChangeStatusCreate(GwSystem.current_player_index, -var_s1_3);
    MBCoinTakeCreateSound(GwSystem.current_player_index, -var_s1_3, 0);
    MBCoinChangeStatusCreate(var_s4, -var_s1_3);
    MBCoinTakeCreateSound(var_s4, -var_s1_3, 1);
    HuPrcSleep(0x1E);
    MBDlgWinExecY(6, 0x4004, 0x88);

    if (MBPlayerComCheck(-1) != 0) {
        if ((MBPlayerComCheck(var_s4) != 0)) {
            if (*showComMinigames != 0) {
                GwSystem.unk_58 = var_s4;
                GwSystem.playerIndexVisitingBowser = var_s1_3;
                return 2;
            }
        }
    }

    mbItemBtnF = 0;
    func_800F7108_shared_board();
    temp_s0_2 = func_800E11C0_shared_board(GwSystem.current_player_index, var_s4);
    HuPrcChildLink(HuPrcCurrentGet(), temp_s0_2);
    HuPrcChildWait();

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        GwPlayer[i].group = 2;
    }

    arg0->group = 0;
    GwPlayer[var_s4].group = 1;
    GwSystem.unk_58 = var_s4;
    GwSystem.playerIndexVisitingBowser = var_s1_3;

    mbGameMode = (arg1 == 0) ? 8 : 9;

    D_80105664_shared_board = 2;
    D_80105666_shared_board = 1;
    D_800D1380_main = 8;
    return 1;
}

/**
 * @brief Gets the current game phase based on the given turn number.
 *
 * This function determines which phase of the game the given turn falls into:
 * early game (0), mid game (1), or late game (2). The thresholds for these
 * phases depend on the total number of turns set for the game and are defined
 * in a lookup table.
 *
 * If the special value `CUR_TURN` is passed, the current turn from GwSystem
 * is used.
 *
 * @param turn The turn number to check. Pass `CUR_TURN` to use the current game turn.
 * @return An integer representing the game phase:
 *         - 0: Early game
 *         - 1: Mid game
 *         - 2: Late game
 */
s32 BoardGetTurnTier(s32 turn) {
    u32 turnTierIndex;
    s32 i;
    GW_SYSTEM *system = &GwSystem;

    if (turn == CUR_TURN) {
        turn = system->current_turn;
    }

    turnTierIndex = (system->total_turns / 5) - 2;

    if (system->total_turns < turn) {
        turn = system->total_turns;
    }

    if (turnTierIndex >= 9) {
        turnTierIndex = 0;
    }

    for (i = 0; i < 3; i++) {
        if (D_80101DE8_shared_board[turnTierIndex][i] > turn) {
            break;
        }
    }

    return i;
}

static s32 func_800FB624_shared_board(GW_PLAYER* arg0) {
    char sp20[16];
    omObjData* sp34;
    omObjData* kuriboObj;
    s32 sp3C;
    s32 sp44;
    s32 sp4C;
    Object* temp_v0;
    Object* temp_v0_2;
    f32 var_f20;
    f32 var_f22;
    omObjData* temp_v0_3;
    s32 var_a0;
    s32 var_a1;
    s32 turnTier; // s3
    s32 var_s1;
    s32 var_s2;
    s32 var_s3;
    s32 var_s4;
    s32 var_s5;
    s32 var_s6;
    void* temp_v0_4;
    s32 i; // s0
    s32 j;
    u8 *table;
    
    mbItemBtnF = 0;
    func_8004A670_main(0xF);
    HuPrcSleep(0x10);
    D_800C9930_main = func_8004A5C4_main(0x3E);
    mbItemBtnF = 1;
    
    mbBattleKuriboMdl = MBModelCreate(0xCU, 0);
    Hu3DModelScaleSet(mbBattleKuriboMdl->omObj1->model[0], 0.0f, 0.0f, 0.0f);
    func_8001FDE8_main(mbBattleKuriboMdl->omObj1->model[0]);
    
    mbBattleKuriboMdl->flags |= 1;
    mbBattleKuriboUpperMdl = MBModelCreate(0x1F, 0);
    
    Hu3DModelScaleSet(mbBattleKuriboUpperMdl->omObj1->model[0], 0.0f, 0.0f, 0.0f);
    HuVecCopy3F(&mbBattleKuriboMdl->coords, &arg0->player_obj->coords);
    HuVecCopy3F(&mbBattleKuriboUpperMdl->coords, &arg0->player_obj->coords);
    
    mbBattleKuriboMdl->velocity.x = 120.0f;
    mbBattleKuriboUpperMdl->velocity.x = 122.0f;
    
    while (mbBattleKuriboMdl->velocity.x > 30.0f) {
        HuPrcVSleep();
        mbBattleKuriboMdl->velocity.x -= 1.0f;
        mbBattleKuriboUpperMdl->velocity.x -= 1.0f;
    }
    
    kuriboObj = omAddObj(0x1F4, 0, 0, -1, MBBattleKuriboObjMain);
    kuriboObj->rot.x = 180.0f;

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if (GwPlayer[i].coin >= 10) {
            break;
        }
    }
    
    HuAudFXPlay(0x25B);
    var_s6 = 0;
    if (i == MB_MAX_PLAYERS) {
        func_800EC5EC_shared_board(6, 0x1301, 0x88);
    } else {
        turnTier = BoardGetTurnTier(-1);
        for (i = 0, var_s4 = 0; i < MB_MAX_PLAYERS; i++) {
            if (var_s4 < GwPlayer[i].coin) {
                var_s4 = GwPlayer[i].coin;
            }
        }
        
        for (j = 0; j < 3; j++) {
            if (var_s4 < D_80101DE0_shared_board[j]) {
                break;
            }
        }

        
        
        var_s2 = MBRand(100.0f);
        
        for (i = 0, table = D_80101E50_shared_board[j][0][0]; i < 5; i++) {
            if (var_s2 < table[(turnTier * 5) + i]) {
                break;
            }
        }

        var_s4 = D_80101E44_shared_board[i];
        var_s6 = i;
        sp3C = MBRandCheck100(10);
        if ((turnTier == 0) && ((MBRandCheck100(30)) != 0)) {
            var_s6 = 0;
            var_s4 = D_80101E44_shared_board[0];
        }
        func_800EC980_shared_board(6, 0x1300, NULL, 0, 0, 0, 0, 0x88);
        func_800ECA38_shared_board();
        MBDlgWinClose();
        MBDlgWinKill();
        var_s5 = HuSprGrpCreate(2U, 2U);
        temp_v0_4 = DataRead(0x130113);
        sp44 = func_80055810_main(temp_v0_4);
        DataClose(temp_v0_4);
        func_80055024_main(var_s5, 0, sp44, 0);
        HuSprPriSet(var_s5, 0, 0x14U);
        func_800550F4_main(var_s5, 0, 0);
        HuSprAttrSet(var_s5, 0, 0x1000);
        func_80054904_main(var_s5, 0, 0xA0, 0x78);
        func_800554C4_main(var_s5, 0, -1);
        temp_v0_4 = DataRead(0x130114);
        sp4C = func_80055810_main(temp_v0_4);
        DataClose(temp_v0_4);
        func_80055024_main(var_s5, 1, sp4C, 0);
        HuSprPriSet(var_s5, 1, 0xAU);
        func_800550F4_main(var_s5, 1, 1);
        HuSprAttrSet(var_s5, 1, 0x1000);
        func_80054904_main(var_s5, 1, -0x28, 0x10);
        func_800554C4_main(var_s5, 1, 0);
        
        for (var_f20 = 0.0f; var_f20 < 1.0f; var_f20 += 0.1f) {
            HuSprScaleSet(var_s5, 0, var_f20, var_f20);
            HuSprScaleSet(var_s5, 1, var_f20, var_f20);
            HuPrcVSleep();            
        }
        
        HuSprScaleSet(var_s5, 0, 1.0f, 1.0f);
        HuSprScaleSet(var_s5, 1, 1.0f, 1.0f);
        i = 0;  /* current index */
        var_s3 = 0;    /* tick counter */
        var_s1 = 1;  /* spin speed */
        var_s2 = 0;    /* slowdown counter */
    
        for (;; HuPrcVSleep()) {
            /* Update sprite to current index */
            func_80055024_main(var_s5, 1, sp4C, i);
    
            /* Count frames before next tick */
            var_s3++;
            if (var_s3 > var_s1) {
                var_s3 = 0;
                HuAudFXPlay(0x116);
    
                i++;
                if (i >= 5) {
                    i = 0;
                }
            }
    
            /* Slow down the spinner gradually */
            var_s2++;
            if (var_s2 < 9) {
                continue;
            }
            var_s2 = 0;
            var_s1++;
            if (var_s1 <= 6) {
                continue;
            }
            var_s1 = 6;
            
            /* Check stop conditions */
            if (sp3C != 0) {
                /* Overshoot case (10% RNG) */
                if (var_s6 != 4) {
                    if (i == (var_s6 + 1)) {
                        break;
                    }
                } else {
                    if (i == 0) {
                        break;
                    }
                }
            } else {
                /* Normal stop */
                if (i == var_s6) {
                    break;
                }
            }
        }
        func_80055024_main(var_s5, 1, sp4C, i);
        HuPrcSleep(0x14);
        if (sp3C != 0) {
            HuAudFXPlay(0x116);
            func_80055024_main(var_s5, 1, sp4C, var_s6);
            HuPrcSleep(0xA);
        }

        if (var_s6 != 0) {
            HuAudFXPlay(0x117);
        } else {
            HuAudFXPlay(0x118);
        }
        
        func_800F6A88_shared_board(var_s5, 0);
        func_80054904_main(var_s5, 1, 0x78, 0x88);
        if (var_s6 == 4) {
            for (var_f22 = 0.0f; var_f22 <= 1080.0f; var_f22 += 50.0f) {
                var_f20 = (HuMathSin(var_f22) * 0.5f) + 1.0f;
                HuSprScaleSet(var_s5, 1, var_f20, var_f20);
                HuPrcVSleep();                
            }

            for (; var_f22 <= 1440.0f; var_f22 += 40.0f) {
                var_f20 = (HuMathSin(var_f22) * 0.5f) + 1.0f;
                HuSprScaleSet(var_s5, 1, var_f20, var_f20);
                HuPrcVSleep();               
            }
        } else {
            for (var_f22 = 0.0f; var_f22 <= 720.0f; var_f22 += 40.0f) {
                var_f20 = (HuMathSin(var_f22) * 0.5f) + 1.0f;
                HuSprScaleSet(var_s5, 1, var_f20, var_f20);
                HuPrcVSleep();                
            }

            for (; var_f22 <= 1080.0f; var_f22 += 30.0f) {
                var_f20 = (HuMathSin(var_f22) * 0.5f) + 1.0f;
                HuSprScaleSet(var_s5, 1, var_f20, var_f20);
                HuPrcVSleep();               
            }
        }
        HuSprScaleSet(var_s5, 1, 1.0f, 1.0f);
        func_800F6A88_shared_board(var_s5, 2U);
        func_80054904_main(var_s5, 1, -0x28, 0x10);
        HuPrcSleep(0x1E);
        
        for (var_f20 = 1.0f; var_f20 > 0.0f; var_f20 -= 0.1f) {
            HuSprScaleSet(var_s5, 0, var_f20, var_f20);
            HuSprScaleSet(var_s5, 1, var_f20, var_f20);
            HuPrcVSleep();            
        }

        HuSprScaleSet(var_s5, 0, 0.0f, 0.0f);
        HuSprScaleSet(var_s5, 1, 0.0f, 0.0f);
        HuSprGrpKill(var_s5);
        HuSprKill(sp44);
        HuSprKill(sp4C);
        HuPrcSleep(3);

        if (var_s4 == 0) {
            var_s3 = -1;
            var_s6 = 0;
            func_800EC980_shared_board(6, 0x1304, NULL, 0, 0, 0, 0, 0x88);
            func_800ECA38_shared_board();
            MBDlgWinClose();
            MBDlgWinKill();
        } else {
            var_s6 = 1;
            var_s3 = -1;
            var_a0 = 0;
            for (i = 0; i < MB_MAX_PLAYERS; i++) {
                if (GwPlayer[i].coin >= var_s4) {
                    var_s3 = i;
                    break;
                } else {
                    if (var_a0 < GwPlayer[i].coin) {
                        var_s3 = i;
                        var_a0 = GwPlayer[i].coin;
                    }
                }
            }
            
            var_s2 = 0; //coin total
            for (i = 0; i < MB_MAX_PLAYERS; i++) {
                if (GwPlayer[i].coin < var_s4) {
                    GwPlayer[i].checkCoin = GwPlayer[i].coin;
                    var_s2 += GwPlayer[i].coin;
                    MBCoinChangeStatusCreate(i, -GwPlayer[i].coin);
                    if (i == var_s3) {
                        MBCoinTakeCreateSound(i, -GwPlayer[i].coin, 1);
                    } else {
                        MBCoinTakeCreateSound(i, -GwPlayer[i].coin, 0);
                    }
                } else {
                    GwPlayer[i].checkCoin = var_s4;
                    var_s2 += var_s4;
                    MBCoinChangeStatusCreate(i, -var_s4);
                    if (i == var_s3) {
                        MBCoinTakeCreateSound(i, -var_s4, 1);
                    } else {
                        MBCoinTakeCreateSound(i, -var_s4, 0);
                    }
                }
            }

            HuPrcSleep(0x1E);
            sprintf(sp20, "%d", var_s2);
            if (var_s2 < (var_s4 * 4)) {
                func_800EC980_shared_board(6, 0x1303, (u32)sp20, 0, 0, 0, 0, 0x88);
                func_800ECA38_shared_board();
            } else {
                func_800EC980_shared_board(6, 0x1302, (u32)sp20, 0, 0, 0, 0, 0x88);
            }
            func_800ECA38_shared_board();
            MBDlgWinClose();
            MBDlgWinKill();
            D_800CC698_main = var_s2;
        }
    }
    
    if (var_s6 == 0) {
        omDelObj(kuriboObj);
        while (mbBattleKuriboMdl->velocity.x <= 120.0f) {
            HuPrcVSleep();
            mbBattleKuriboMdl->velocity.x += 3.0f;
            mbBattleKuriboUpperMdl->velocity.x += 3.0f;
        }
        MBModelKill(mbBattleKuriboMdl);
        MBModelKill(mbBattleKuriboUpperMdl);
        mbItemBtnF = 0;
        func_8004A7C4_main(0x1E);
        HuPrcSleep(0xF);
        func_8004A72C_main(0x1E);
        mbItemBtnF = 1;
        D_800C9930_main = -1;
        D_800CE206 = 0;
        omPrcSetStatBit(omAddPrcObj(func_800F9C68_shared_board, 0U, 0, 0), 0xA0);
        HuPrcSleep(2);
        return 0;
    } else {
        return 1;
    }
}

static void func_800FC3D0_shared_board(void) {
    s32 var_a0;
    s32 var_s1;
    s32 i, j;

    MBDlgWinExecY(6, 0x1305, 0x88);
    var_s1 = 0;
    var_a0 = 0;
    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if (var_a0 < GwPlayer[i].checkCoin) {
            var_a0 = GwPlayer[i].checkCoin;
            var_s1 = i;
        }
    }

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        if (GwPlayer[i].checkCoin != 0) {
            MBCoinChangeStatusCreate(i, GwPlayer[i].checkCoin);
        }
        if (i == var_s1) {
            MBCoinTakeCreateSound(i, GwPlayer[i].checkCoin, 1);
        } else {
            if (GwPlayer[i].checkCoin != 0) {
                MBCoinTakeCreateSound(i, GwPlayer[i].checkCoin, 0);
            }
        }
    }

    MBBattleKuriboEnd();
}

static void func_800FC4E0_shared_board(void) {
    if (GwPlayer[GwSystem.playerIndexVisitingBowser].gameCoin != 0) {
        if (GwPlayer[GwSystem.playerIndexVisitingBowser].bonusCoin != 0) {
            MBCoinChangeStatusCreate(GwSystem.playerIndexVisitingBowser, GwPlayer[GwSystem.playerIndexVisitingBowser].gameCoin * GwPlayer[GwSystem.playerIndexVisitingBowser].bonusCoin);
            MBCoinTakeCreateSound(GwSystem.playerIndexVisitingBowser, GwPlayer[GwSystem.playerIndexVisitingBowser].gameCoin * GwPlayer[GwSystem.playerIndexVisitingBowser].bonusCoin, 1);
        }
    }
    HuPrcSleep(30);
}

static void MBBlockMasuUpdate(void) {
    s32 i;

    if (GWBoardFlagCheck(0xF)) {
        while (kakusiMasuCoin == -1 || kakusiMasuCoin == kakusiMasuStar || kakusiMasuCoin == kakusiMasuItem) {
            kakusiMasuCoin = MBMasuKakusiBlockGet(D_800D03FC);
            D_800D03FC++;
            for (i = 0; i < ARRAY_COUNT(kakusiMasuCoinHis); i++) {
                if (kakusiMasuCoin == kakusiMasuCoinHis[i]) {
                    break;
                }
            }
            if (i != ARRAY_COUNT(kakusiMasuCoinHis)) {
                kakusiMasuCoin = -1;
            }
        }
        while (kakusiMasuStar == -1 || kakusiMasuCoin == kakusiMasuStar || kakusiMasuItem == kakusiMasuStar) {
            kakusiMasuStar = MBMasuKakusiBlockGet(D_800CE208);
            D_800CE208++;
            for (i = 0; i < ARRAY_COUNT(kakusiMasuStarHis); i++) {
                if (kakusiMasuStar == kakusiMasuStarHis[i]) {
                    break;
                }
            }
            if (i != ARRAY_COUNT(kakusiMasuStarHis)) {
                kakusiMasuStar = -1;
            }
        }

        while (kakusiMasuItem == -1 || kakusiMasuCoin == kakusiMasuItem || kakusiMasuStar == kakusiMasuItem) {
            kakusiMasuItem = MBMasuKakusiBlockGet(D_800CDD68_main);
            D_800CDD68_main++;
            for (i = 0; i < ARRAY_COUNT(kakusiMasuItemHis); i++) {
                if (kakusiMasuItem == kakusiMasuItemHis[i]) {
                    break;
                }
            }
            if (i != ARRAY_COUNT(kakusiMasuItemHis)) {
                kakusiMasuItem = -1;
            }
        }
    }
}

void func_800FC7C8_shared_board(void) {
    s32 var_s0 = 0xA0;

    if (D_80101E60_shared_board == NULL) {
        D_80101E60_shared_board = func_800E210C_shared_board(2, var_s0, 1);
    }
    if ((D_80101E64_shared_board == NULL) && !(GwSystem.cur_player_used_item & 1)) {
        var_s0 += 14;
        D_80101E64_shared_board = func_800E210C_shared_board(0xD, var_s0, 1);
    }
    if (D_80101E68_shared_board == NULL) {
        var_s0 += 14;
        D_80101E68_shared_board = func_800E210C_shared_board(3, var_s0, 1);
    }
    if (D_80101E6C_shared_board == NULL) {
        var_s0 += 14;
        D_80101E6C_shared_board = func_800E210C_shared_board(0xB, var_s0, 1);
    }
}

void func_800FC8A4_shared_board(void) {
    if (D_80101E60_shared_board != NULL) {
        MBHelpWinKill(D_80101E60_shared_board);
        D_80101E60_shared_board = NULL;
    }
    if (D_80101E64_shared_board != NULL) {
        MBHelpWinKill(D_80101E64_shared_board);
        D_80101E64_shared_board = NULL;
    }
    if (D_80101E68_shared_board != NULL) {
        MBHelpWinKill(D_80101E68_shared_board);
        D_80101E68_shared_board = NULL;
    }
    if (D_80101E6C_shared_board != NULL) {
        MBHelpWinKill(D_80101E6C_shared_board);
        D_80101E6C_shared_board = NULL;
    }
}

void func_800FC938_shared_board(void) {
    mbGameMode = 0xA;
    D_800D1380_main = 8;
    func_800FC8A4_shared_board();
}

void func_800FC968_shared_board(void) {
    mbGameMode = 0xC;
    D_800D1380_main = 8;
    func_800FC8A4_shared_board();
}

void func_800FC998_shared_board(void) {
    mbGameMode = 0xD;
    D_80105664_shared_board = 2;
    D_80105666_shared_board = 1;
    D_800D1380_main = 9;
    func_800FC8A4_shared_board();
}

void func_800FC9E0_shared_board(void) {
    mbGameMode = 0xB;
    func_800FC8A4_shared_board();
    func_800FF7C4_shared_board(0x52, 0, 0xB);
}

void func_800FCA14_shared_board(void) {
    mbGameMode = 0xF;
    D_800D1380_main = 0xB;
    D_80105666_shared_board = 0;
    func_800FC8A4_shared_board();
}

void func_800FCA4C_shared_board(void) {
    mbGameMode = 0xE;
    D_800D1380_main = 3;
    func_800FC8A4_shared_board();
}

static void MBMain(void) {
    Process *telopProc;
    s32 i;
    s16 temp_v0_2;
    s32 temp_v0_3;
    GW_PLAYER *temp_v0_4;
    s16 var_s5;
    SpaceData *space;
    Vec sp20;
    Process *temp_s1;
    u8 temp_v1_2;
    u8 temp_v1_3;
    Process *temp_s1_2;
    s16 temp_v0_9;
    s32 temp_v0_16;
    s32 var_v1;
    s32 var_s0;
    s16 temp_v0_18;
    GW_PLAYER *temp_s0_2;
    s32 temp_v0_20;
    s32 temp_s2;
    s32 temp_s0_3;
    s32 temp_s0_4;
    s32 temp_a0;
    s32 temp_s2_2;
    void *temp_v0_21;
    s32 temp_s3_3;
    Object *temp_v0_22;
    Process *temp_s1_5;
    s32 temp_a0_4;
    u32 var_v1_3;
    s32 temp_v1_8;
    s32 temp_a0_3;
    s32 var_s1_12;
    s32 var_s0_2;
    GW_SYSTEM *system = &GwSystem;
    s16 sp30[2];
    s16 sp38[2];
    s16 sp40[2];
    char sp48[16]; // unknown buffer size
    s32 flag;
    s32 state;
    s32 eight;

    telopProc = HuPrcCurrentGet();

    while (1) { // Main game loop
        if ((mbGameMode == 0 || mbGameMode == 1) && system->current_player_index == 0) {
            // Initialize all player colors
            for (i = 0; i < 4; i++) {
                MBPlayerGet(i)->color = 0;
                func_800F70F8_shared_board();
            }

            // Check if we're in the last 5 turns
            if (system->total_turns >= 0) {
                short temp_v0 = (system->total_turns - system->current_turn) + 1;
                if (temp_v0 < 6) {
                    if (temp_v0 > 0) {
                        GWBoardFlagSet(2); // Last 5 turns flag
                    }
                }
            }

            if (!GWBoardFlagCheck(1) && system->current_turn >= 2) {
                switch (system->save_mode) {
                    case 1:
                        system->save_mode = 2;
                    case 0:
                        _SetFlag(0);
                        GWBoardFlagSet(0);

                        func_80060394_main(1, sp30, 0x5900);
                        temp_v0_2 = func_8005A968_main(0, 0, sp30[0], sp30[1], 0, 2);
                        func_8005BEE0_main(temp_v0_2, 0);
                        func_8005B43C_main(temp_v0_2, 0x5900, -1, -1);
                        func_8005BDFC_main(temp_v0_2, 0);

                        while (func_8005E0C0_main(temp_v0_2) != 0) {
                            HuPrcVSleep();
                        }

                        if (system->playMode & 4) {
                            _SetFlag(0x18);
                            _SetFlag(0x28);
                            func_8005035C_main(1);
                        } else {
                            _SetFlag(0x16);
                            func_800500E0_main(1);
                        }

                        func_8005F364_main(temp_v0_2);
                        func_80060394_main(1, sp30, 0x5901);
                        temp_v0_3 = func_8005A968_main(0, 0, sp30[0], sp30[1], 0, 2);
                        func_8005BEE0_main(temp_v0_3, 0);
                        func_8005B43C_main(temp_v0_3, 0x5901, -1, -1);
                        func_8005BDFC_main(temp_v0_3, 0);
                        HuPrcSleep(0xF);
                        func_8005F364_main(temp_v0_3);
                        break;
                }
            }

            GWBoardFlagClear(1);
            D_800A12D4_main = 0;
            D_80105660_shared_board = MBMasuEventCall(-2, 7);
        }

        D_800A12D4_main = 1;

        i = 0;
        eight = 8; // definitely a hack
        for (; i < 4; i++) {
            MBStatusColorSet(i, MBPlayerGet(i)->color);
            MBPlayerPosFixSet(i, 1);

            state = mbGameMode;

            if (state == eight) {
                // skip
            } else if (state < 8) {
                flag = 1;
                GwSystem.cur_player_used_item &= ~flag;
            } else if (state >= 0x10) {
                flag = 1;
                GwSystem.cur_player_used_item &= ~flag;
            } else if (state < 0xA) {
                flag = 1;
                GwSystem.cur_player_used_item &= ~flag;
            }
        }

        MBBlockMasuUpdate();
        temp_v0_4 = MBPlayerGet(system->current_player_index);
        var_s5 = MBMasuLinkMasuIdGet(temp_v0_4->clink, temp_v0_4->cidx);
        space = MBMasuGet(var_s5);
        MBMasuCurSet(var_s5);

        state = mbGameMode;

        if (state == 0 || state == 1) {
            goto temp;
        }

        switch (state) {
            case 8:
            case 9:
            case 0xC:
                MBBattleKuriboStart(temp_v0_4, 1);
                break;

            case 0x12:
                i = 0;
                for (; i < 4; i++) {
                    if (GwPlayer[i].bonusCoin != 3)
                        break;
                }
                if (i >= 4) {
                    MBBattleKuriboStart(temp_v0_4, 1);
                }
                break;

            default:
                break;
        }

        while (WipeStatGet() != 0) {
            HuPrcVSleep();
        }

        mbItemBtnF = 1;

        switch (mbGameMode) {
            case 12:
                MBKettouResultExec();
                func_800F88D0_shared_board();
                goto label654;

            case 13: // Bank space
                func_800FC4E0_shared_board();

            case 15: // Some event
                func_800F88D0_shared_board();
                goto label654;
            case 14:
                MBItemKoopaNKinokoExec();
                MBItemKoopaRKinokoExec();
                func_800F88D0_shared_board();
                goto label654;

            case 11:
                mbGameMode = 0xA;
                MBItemMajinLampStarExec();
                MBVecForwardSet(&sp20);
                func_800ED128_shared_board(&temp_v0_4->player_obj->rot, &sp20,
                                                  &temp_v0_4->player_obj->rot, 8);
                HuPrcSleep(8);
                goto label654;

            temp:
                MBMasuEventCall(-4, 7);
                // Hide all player models
                func_8001FDE8_main(MBPlayerGet(0)->player_obj->omObj1->model[0]);
                func_8001FDE8_main(MBPlayerGet(1)->player_obj->omObj1->model[0]);
                func_8001FDE8_main(MBPlayerGet(2)->player_obj->omObj1->model[0]);
                func_8001FDE8_main(MBPlayerGet(3)->player_obj->omObj1->model[0]);

                MBPlayerMotionSet(-1, -1, 2);
                MBCameraFocusModeSet(1);

                if (system->current_player_index == 4) {
                    func_800E648C_shared_board();
                }

                // Camera setup
                if (D_800A12CC_main == 0) {
                    D_800C9938_main = 1.0f;
                    MBCameraZoomSet(1.0f);
                    HuPrcChildLink(telopProc, func_800F29EC_shared_board());
                } else if (system->current_player_index == 4 || system->current_player_index == 0) {
                    D_800C9938_main = 1.0f;
                    MBCameraZoomSet(1.0f);
                    HuPrcChildLink(telopProc, func_800F29EC_shared_board());
                } else {
                    D_800C9938_main = 1.0f;
                    HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.0f));
                }

                HuPrcChildWait();
            default:
                HuPrcSleep(4);
                MBPlayerVibrate(-1, 3);
                HuPrcChildLink(telopProc, MBTelopCreate(system->current_player_index));
                HuPrcChildWait();
                MBMasuEventCall(-5, 7);
                goto label654;
            case 6:
            case 16:
                HuPrcSleep(0x1E);
                goto label2444;

            case 9:
                MBKettouResultExec();
                goto label2444;

            case 10:
            label654:
                // STATE 10: Dice roll and movement
                func_800FC7C8_shared_board();
                D_800C9938_main = 1.3f;
                HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.3f));
                temp_s1 = func_800DD6C4_shared_board(system->current_player_index);
                HuPrcChildWait();
                HuPrcSleep(1);

                if (system->current_player_index != 4) {
                    mbCameraBtnF = 1;
                    mbItemBtnF = 1;
                    D_800CDD64_main = 1;
                    D_800CC4A0_main = 1;
                }

                HuPrcChildLink(telopProc, temp_s1);
                HuPrcChildWait();
                func_800FC8A4_shared_board();

                mbCameraBtnF = 0;
                D_800CDD64_main = 0;
                mbWalkNum = func_800DCD9C_shared_board(system->current_player_index);

                if (D_80105660_shared_board == 0 && (temp_v0_4->rev & 0x80)) {
                    temp_v0_4->nlink = temp_v0_4->blink;
                    temp_v0_4->nidx = temp_v0_4->bidx;
                    temp_v0_4->rev ^= 1;
                }

                var_s5 = MBMasuCurGet();
                D_80105660_shared_board = MBMasuEventCall(var_s5, 9);
                MBPlayerMotionSet(-1, 0, 2);

                while (mbWalkNum != 0) {
                    short temp_s3;
                    short temp_s0;
                    MBMasuEventCall(var_s5, 8);

                    // Save current position
                    temp_s3 = temp_v0_4->clink;
                    temp_s0 = temp_v0_4->cidx;

                    // Update to next position
                    temp_v0_4->clink = temp_v0_4->nlink;
                    temp_v0_4->cidx = temp_v0_4->nidx;

                    // Update walk speed animations
                    for (i = 0; i < 4; i++) {
                        MBPlayerPosFixSet(i, (system->walk_speed * 5) + 5);
                    }

                    // Restore position temporarily
                    temp_v0_4->clink = temp_s3;
                    temp_v0_4->cidx = temp_s0;

                    // Check if next space requires jumping
                    space = MBMasuGet(MBMasuLinkMasuIdGet(temp_v0_4->nlink, temp_v0_4->nidx));

                    if ((space->unk_02 & 0x8000) || func_800F9A68_shared_board(system->current_player_index)) {
                        // Jump animation
                        func_8004ACE0_main(0x3A, system->current_player_index);
                        MBPlayerMotionSet(-1, 2, 0);

                        switch (system->walk_speed) {
                            case 0:
                                temp_v0_4->player_obj->velocity.y = 8.0f;
                                temp_v0_4->player_obj->velocity.z = -2.0f;
                                MBPlayerPosMoveSet(system->current_player_index, 7);
                                break;
                            case 1:
                                temp_v0_4->player_obj->velocity.y = 4.0f;
                                temp_v0_4->player_obj->velocity.z = -0.5f;
                                MBPlayerPosMoveSet(system->current_player_index, 0xF);
                                break;
                            case 2:
                                temp_v0_4->player_obj->velocity.y = 2.0f;
                                temp_v0_4->player_obj->velocity.z = -0.125f;
                                MBPlayerPosMoveSet(system->current_player_index, 0x1E);
                                break;
                        }
                    } else {
                        // Normal walk animation
                        if (temp_v0_4->player_obj->unk46 != 0) {
                            MBPlayerMotionSet(-1, 0, 2);
                        }

                        switch (system->walk_speed) {
                            case 0:
                                func_8001C92C_main(temp_v0_4->player_obj->omObj1->model[0], 2.0f);
                                MBPlayerPosMoveSet(system->current_player_index, 5);
                                break;
                            case 1:
                                func_8001C92C_main(temp_v0_4->player_obj->omObj1->model[0], 1.0f);
                                MBPlayerPosMoveSet(system->current_player_index, 0xA);
                                break;
                            case 2:
                                func_8001C92C_main(temp_v0_4->player_obj->omObj1->model[0], 0.75f);
                                MBPlayerPosMoveSet(system->current_player_index, 0xF);
                                break;
                        }
                        func_8001C92C_main(temp_v0_4->player_obj->omObj1->model[0], 1.0f);
                    }

                    // Update position
                    temp_v0_4->blink = temp_v0_4->clink;
                    temp_v0_4->bidx = temp_v0_4->cidx;
                    temp_v0_4->clink = temp_v0_4->nlink;
                    temp_v0_4->cidx = temp_v0_4->nidx;

                    // Calculate next index based on direction
                    if (temp_v0_4->rev & 1) {
                        temp_v0_4->nidx = temp_v0_4->nidx - 1;
                    } else {
                        temp_v0_4->nidx = temp_v0_4->nidx + 1;
                    }

                    var_s5 = MBMasuLinkMasuIdGet(temp_v0_4->clink, temp_v0_4->cidx);
                    space = MBMasuGet(var_s5);
                    MBMasuCurSet(var_s5);

                    // Handle passing over certain space types
                    switch (space->space_type) {
                        case SPACE_BLUE:
                        case SPACE_RED:
                        case SPACE_HAPPENING:
                        case SPACE_CHANCE_TIME:
                        case SPACE_ITEM:
                        case SPACE_BANK:
                        case SPACE_BATTLE:
                        case SPACE_BOWSER:
                        case SPACE_GAME_GUY:
                            func_800DBFBC_shared_board(system->current_player_index);
                            mbWalkNum--;
                            func_800EB97C_shared_board(var_s5);
                            break;
                    }

                    // Play appropriate sound effect
                    if (mbWalkNum != 0) {
                        func_800F95FC_shared_board(space->space_type);
                    } else {
                        func_800F965C_shared_board(space->space_type);
                    }

                    MBItemKSuitWalkExec();
                    D_80105660_shared_board = MBMasuEventCall(var_s5, 1);

                    if (D_80105664_shared_board < 0) {
                        continue;
                    }

                    mbGameMode = 2;
                    HuPrcSleep(-1);
                    case 2:
                        if (D_80105660_shared_board & 1) {
                            var_s5 = MBMasuCurGet();
                            D_80105660_shared_board = MBMasuEventCall(var_s5, 2);
                            if (D_80105664_shared_board >= 0) {
                                mbGameMode = 3;
                                HuPrcSleep(-1);
                            }
                        }
                        /* fallthrough */
                    case 3:
                        if (D_80105660_shared_board & 2) {
                            mbWalkNum = 0;
                            continue;
                        }
                        func_800DB884_shared_board(system->current_player_index);
                        HuPrcSleep(0xA);
                }

                if (system->current_board_index == 3) {
                    GwSystem.boardData.halfWordBytes[4] = 0;
                }

                // Handle reverse direction cleanup
                if (temp_v0_4->rev & 0x80) {
                    if (temp_v0_4->rev & 1) {
                        // Swap blink/nlink and bidx/nidx
                        temp_v1_2 = temp_v0_4->blink;
                        temp_v0_4->blink = temp_v0_4->nlink;
                        temp_v0_4->nlink = temp_v1_2;

                        temp_v1_3 = temp_v0_4->bidx;
                        temp_v0_4->bidx = temp_v0_4->nidx;
                        temp_v0_4->nidx = temp_v1_3;
                    }
                    temp_v0_4->rev &= ~(0x80 | 0x1);
                }

                MBPlayerMotionSet(-1, -1, 2);
                var_s5 = MBMasuLinkMasuIdGet(temp_v0_4->clink, temp_v0_4->cidx);

                switch (MBMasuGet(var_s5)->space_type) {
                    case SPACE_HAPPENING:
                        MBPlayerVibrate(-1, 3);
                        temp_v0_4->stats.prize.hatenaPrize++;
                        break;
                    case SPACE_RED:
                        MBPlayerVibrate(-1, 1);
                        temp_v0_4->stats.prize.redPrize++;
                        break;
                    case SPACE_BLUE:
                        temp_v0_4->stats.prize.bluePrize++;
                        break;
                    case SPACE_CHANCE_TIME:
                        MBPlayerVibrate(-1, 4);
                        temp_v0_4->stats.prize.eventPrize++;
                        break;
                    case SPACE_GAME_GUY:
                        MBPlayerVibrate(-1, 4);
                        temp_v0_4->stats.prize.gamblePrize++;
                        break;
                    case SPACE_BOWSER:
                        MBPlayerVibrate(-1, 2);
                        temp_v0_4->stats.prize.kupaPrize++;
                        break;
                    case SPACE_BATTLE:
                        MBPlayerVibrate(0, 2);
                        MBPlayerVibrate(1, 2);
                        MBPlayerVibrate(2, 2);
                        MBPlayerVibrate(3, 2);
                        temp_v0_4->stats.prize.battlePrize++;
                        break;
                    case SPACE_ITEM:
                        MBPlayerVibrate(-1, 2);
                        temp_v0_4->stats.prize.itemPrize++;
                        break;
                    case SPACE_BANK:
                        MBPlayerVibrate(-1, 3);
                        temp_v0_4->stats.prize.bankPrize++;
                        break;
                }

                mbGameMode = 4;
                D_80105660_shared_board = MBMasuEventCall(var_s5, 3);

                if (D_80105664_shared_board < 0) {
                    goto labelE5C;
                }
                HuPrcSleep(-1);
                /* fallthrough */
            case 4:
                // STATE 4 & 5: Additional space handling
                if (D_80105660_shared_board & 1) {
                    D_80105660_shared_board = MBMasuEventCall(MBMasuCurGet(), 4);

                    if (D_80105664_shared_board >= 0) {
                        mbGameMode = 5;
                        HuPrcSleep(-1);
                    }
                }
                /* fallthrough */
            case 5:
                HuPrcSleep(30);
            labelE5C:

                // Return player to space position
                MBVecForwardSet(&sp20);
                func_800ED1E4_shared_board(&temp_v0_4->player_obj->rot, &sp20,
                                                  &temp_v0_4->player_obj->rot, 8);

                var_s5 = MBMasuLinkMasuIdGet(temp_v0_4->clink, temp_v0_4->cidx);
                space = MBMasuGet(var_s5);

                if (kakusiMasuStar == var_s5 || D_800D4080_main != 0) {
                    MBPlayerVibrate(-1, 5);
                    HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.6f));
                    temp_s1 = func_800DDDD4_shared_board(system->current_player_index);
                    func_800DE858_shared_board();

                    // Display message window
                    func_80060394_main(1, sp30, 0x3F00);
                    temp_v0_9 = func_80061188_main(-1, 0xA0 - (sp30[0] / 2), 0x96, sp30[0], sp30[1], 0);
                    func_8005B43C_main(temp_v0_9, 0x3F00, -1, -1);
                    func_80061388_main(temp_v0_9);
                    func_800600C0_main(temp_v0_9, 1);
                    HuPrcChildWait();

                    func_800600C0_main(temp_v0_9, 0);
                    func_800EDC58_shared_board(temp_v0_9, temp_v0_4->turn);
                    func_80061A5C_main(temp_v0_9, 0);
                    func_8005F364_main(temp_v0_9);
                    func_800DE868_shared_board();

                    // Wait for player input
                    while (!(func_800DE84C_shared_board())) {
                        HuPrcVSleep();
                    }

                    // Display follow-up message
                    func_80060394_main(1, sp30, 0x3F01);
                    temp_v0_9 = func_80061188_main(-1, 0xA0 - (sp30[0] / 2), 0x96, sp30[0], sp30[1], 0);
                    func_8005B43C_main(temp_v0_9, 0x3F01, -1, -1);
                    func_80061388_main(temp_v0_9);
                    func_800EDC58_shared_board(temp_v0_9, temp_v0_4->turn);
                    func_80061A5C_main(temp_v0_9, 0);
                    func_8005F364_main(temp_v0_9);
                    func_800DE868_shared_board();

                    HuPrcChildLink(telopProc, temp_s1);
                    HuPrcChildWait();
                    HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.3f));
                    HuPrcChildWait();
                    HuPrcSleep(1);

                    // Update star block history
                    kakusiMasuStarHis[kakusiMasuStarHisIdx++] = kakusiMasuStar;
                    kakusiMasuStarHisIdx = kakusiMasuStarHisIdx % 10;
                    kakusiMasuStar = -1;
                    MBBlockMasuUpdate();
                }

                // COIN BLOCK HANDLING (similar structure to star block)
                if (kakusiMasuCoin == var_s5 || D_800D2130_main != 0) {
                    MBPlayerVibrate(-1, 5);
                    HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.6f));
                    temp_s1 = func_800DE7E4_shared_board(system->current_player_index);
                    func_800DE858_shared_board();
                    func_80060394_main(1, sp38, 0x3F00);
                    temp_v0_9 = func_80061188_main(-1, 0xA0 - (sp38[0] / 2), 0x96, sp38[0], sp38[1], 0);
                    func_8005B43C_main(temp_v0_9, 0x3F00, -1, -1);
                    func_80061388_main(temp_v0_9);
                    func_800600C0_main(temp_v0_9, 1);
                    HuPrcChildWait();
                    func_800600C0_main(temp_v0_9, 0);
                    func_800EDC58_shared_board(temp_v0_9, temp_v0_4->turn);
                    func_80061A5C_main(temp_v0_9, 0);
                    func_8005F364_main(temp_v0_9);
                    func_800DE868_shared_board();

                    while (!func_800DE84C_shared_board()) {
                        HuPrcVSleep();
                    }

                    HuPrcChildLink(telopProc, temp_s1);
                    HuPrcChildWait();
                    func_80060394_main(1, sp38, 0x3F02);
                    temp_v0_9 = func_80061188_main(-1, 0xA0 - (sp38[0] / 2), 0x96, sp38[0], sp38[1], 0);
                    func_8005B43C_main(temp_v0_9, 0x3F02, -1, -1);
                    func_80061388_main(temp_v0_9);
                    func_800EDC58_shared_board(temp_v0_9, temp_v0_4->turn);
                    func_80061A5C_main(temp_v0_9, 0);
                    func_8005F364_main(temp_v0_9);
                    HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.3f));
                    HuPrcChildWait();
                    HuPrcSleep(1);
                    kakusiMasuCoinHis[kakusiMasuCoinHisIdx++] = kakusiMasuCoin;
                    kakusiMasuCoinHisIdx = kakusiMasuCoinHisIdx % 10;
                    kakusiMasuCoin = -1;
                    MBBlockMasuUpdate();
                }

                // ITEM BLOCK HANDLING
                if (kakusiMasuItem == var_s5 || D_800D1708_main != 0) {
                    short var_s1;
                    short temp_v1_6;
                    MBPlayerVibrate(-1, 5);

                    // Random item selection with weighted probabilities
                    do {
                        var_s1 = 0;
                        temp_v1_6 = MBRand(100.0f) + 1;

                        while (1) {
                            if (temp_v1_6 <= D_80101E70_shared_board[var_s1][1])
                                break;
                            var_s1++;
                        }

                    } while (GwSystem.wackyWatchUsedState != WATCH_AVAILABLE && D_80101E70_shared_board[var_s1][0] == ITEM_WACKY_WATCH);

                    HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.6f));
                    func_800DE414_shared_board(system->current_player_index,
                                                     D_80101E70_shared_board[var_s1][0]);
                    func_800DE858_shared_board();

                    // Display item message window
                    func_80060394_main(1, sp40, 0x3F00);
                    temp_v0_9 = func_80061188_main(-1, 0xA0 - (sp40[0] / 2), 0x96, sp40[0], sp40[1], 0);
                    func_8005B43C_main(temp_v0_9, 0x3F00, -1, -1);
                    func_80061388_main(temp_v0_9);
                    func_800600C0_main(temp_v0_9, 1);
                    HuPrcChildWait();

                    func_800600C0_main(temp_v0_9, 0);
                    func_800EDC58_shared_board(temp_v0_9, temp_v0_4->turn);
                    func_80061A5C_main(temp_v0_9, 0);
                    func_8005F364_main(temp_v0_9);
                    func_800DE868_shared_board();

                    // Wait for input
                    while (!(func_800DE84C_shared_board() & 0xFF)) {
                        HuPrcVSleep();
                    }

                    func_800DE868_shared_board();
                    func_8006022C_main(var_s1 + 0x3F04, 0);

                    // Display item received message
                    func_80060394_main(1, sp40, 0x3F03);
                    temp_v0_9 = func_80061188_main(-1, 0xA0 - (sp40[0] / 2), 0x96, sp40[0], sp40[1], 0);
                    func_8005B6BC_main(temp_v0_9, var_s1 + 0x3F04, 0);
                    func_8005B43C_main(temp_v0_9, 0x3F03, -1, -1);
                    func_80061388_main(temp_v0_9);
                    func_800EDC58_shared_board(temp_v0_9, temp_v0_4->turn);
                    func_80061A5C_main(temp_v0_9, 0);
                    func_8005F364_main(temp_v0_9);

                    // Check if player has empty item slot
                    if (MBItemFindEmpty(system->current_player_index) == -1) {
                        MBDlgResultWinExec(0x3F0A); // Inventory full message
                    }

                    func_800DE858_shared_board();
                    HuPrcSleep(5);

                    // Wait for input
                    while (!(func_800DE84C_shared_board() & 0xFF)) {
                        HuPrcVSleep();
                    }

                    HuPrcSleep(0xA);
                    HuPrcChildLink(telopProc, MBCameraZoomMotStart(1.3f));
                    HuPrcChildWait();
                    HuPrcSleep(1);

                    // Update item block history
                    kakusiMasuItemHis[kakusiMasuItemHisIdx++] = kakusiMasuItem;
                    kakusiMasuItemHisIdx = kakusiMasuItemHisIdx % 10;
                    kakusiMasuItem = -1;
                    MBBlockMasuUpdate();
                }

                // HAPPENING SPACE: Check for duel minigame
                if (space->space_type == 4) {
                    goto label;
                } else {
                    short temp_v0_15;
                    temp_v0_15 = MBKettouPlayerBitGet(1);

                    if (temp_v0_15 == 0) {
                        goto label;
                    } else {
                        GwSystem.unk_58 = temp_v0_15;
                        GwSystem.playerIndexVisitingBowser = (1 << temp_v0_4->turn); //?
                        MBPlayerVibrate(temp_v0_4->turn, 3);

                        temp_v0_16 = MBKettouExec(temp_v0_4, 0);

                        if (temp_v0_16 == 0) {
                            goto label; //?
                            MBKettouResultExec();
                        } else if (temp_v0_16 == 1) {
                            HuPrcSleep(-1);
                        }
                        MBKettouComResultSet();
                    }
                }
                /* fallthrough */
            case 8:
                MBKettouResultExec();
            label:
                MBPlayerMasuColorSet(-1, space->space_type);
                MBStatusColorSet(system->current_player_index, temp_v0_4->color);

                switch (space->space_type) {
                    case SPACE_BLUE:
                        var_v1 = (GWBoardFlagCheck(2) != 0) ? 2 : 1; // Last 5 turns doubles coins
                        var_s0 = var_v1 * 2;
                        var_s0 += var_v1;

                    label181C:
                        MBCoinChangeCreate(system->current_player_index, var_s0);
                        MBCoinTakeCreate(system->current_player_index, var_s0);
                        HuPrcSleep(0x1E);
                        goto label2444;

                    case SPACE_RED:
                        var_v1 = (GWBoardFlagCheck(2) != 0) ? 2 : 1;
                        var_s0 = var_v1 * 2;
                        var_s0 += var_v1;
                        var_s0 = -var_s0;
                        goto label181C;

                    case SPACE_HAPPENING:
                        if (D_80101B6C_shared_board == NULL) {
                            goto label2444;
                        }

                        // Execute board-specific happening event
                        HuPrcChildLink(telopProc, omAddPrcObj(D_80101B6C_shared_board, 0x4800, 0, 0));
                        HuPrcChildWait();

                        // Special handling for board 3
                        if (system->current_board_index == 3 && GwSystem.boardData.halfWordBytes[3] != 0) {
                            GwSystem.boardData.halfWordBytes[3] = 0;
                            goto label654; // Jump back to state 10 (dice roll)
                        }

                        // Check for another duel minigame
                        temp_v0_18 = MBKettouPlayerBitGet(1);

                        if (temp_v0_18 == 0) {
                            goto label2444;
                        }
                        GwSystem.unk_58 = temp_v0_18;
                        GwSystem.playerIndexVisitingBowser = (1 << MBPlayerGet(CUR_PLAYER)->turn); //?
                        MBPlayerVibrate(MBPlayerGet(CUR_PLAYER)->turn, 3);

                        temp_v0_16 = MBKettouExec(temp_v0_4, 1);

                        if (temp_v0_16 == 0) {
                            goto label2444;
                        } else if (temp_v0_16 == 1) {
                            HuPrcSleep(-1);
                        }
                        MBKettouComResultSet();
                        MBKettouResultExec();
                        goto label2444;

                    case SPACE_CHANCE_TIME:
                        mbGameMode = 0x10;

                        // Set up team groups
                        for (i = 0; i < 4; i++) {
                            temp_s0_2 = MBPlayerGet(i);
                            temp_s0_2->group = (i != MBPlayerTurnGet());
                        }

                        // Trigger event minigame
                        func_800FF794_shared_board(0x6A, 0, 5, 1);
                        HuPrcSleep(-1);
                        HuPrcSleep(0x1E);
                        goto label2444;

                    case SPACE_ITEM:
                        mbItemBtnF = 0;
                        HuPrcSleep(8);

                        // Check if last turn
                        if (system->current_turn == system->total_turns) {
                            MBDlgResultWinExec(0x3C01); // Last turn message
                            goto label2444;
                        }

                        // Check if inventory full
                        if (MBItemFindEmpty(system->current_player_index) == -1) {
                            MBDlgResultWinExec(0x3C00); // Inventory full
                            goto label2444;
                        }

                        // Item shop - random outcome
                        temp_v0_20 = MBRand(5.0f);

                        switch (temp_v0_20) {
                            case 0:
                                func_800F85C0_shared_board(); // Shop outcome 1
                                goto label2444;
                            case 1:
                                func_800F85E4_shared_board(); // Shop outcome 2
                                goto label2444;
                            default:
                                // Item minigame option
                                if (MBPlayerComCheck(-1) == 0 || system->show_com_minigames == 0) {
                                    goto label1d70;
                                }
                                // COM item minigame logic
                                WipeCreateOut(6, 0x10);
                                HuPrcSleep(0x11);
                                HuPrcSleep(5);
                                WipeCreateIn(6, 0x10);
                                HuPrcSleep(0x11);

                                temp_s2 = MBRandCheck100(66) ? 0 : 1;

                                // AI decision logic
                                if (!(MBRand(100.0f) >= D_80101C24_shared_board[GwPlayer[system->current_player_index].cpu_difficulty])) {
                                    // AI accepts - determine item
                                    i = 0;
                                    temp_s0_3 = BoardPlayerRankCalc(system->current_player_index);
                                    temp_s0_4 = D_80101D2C_shared_board[temp_s2][temp_s0_3][BoardGetTurnTier(-1)];
                                    temp_a0 = MBRand(100.0f);

                                    // Find item based on weighted table
                                    for (i = 0; i < 8; i++) {
                                        if (D_80101C8C_shared_board[temp_s2][temp_s0_4][i] > temp_a0) {
                                            break;
                                        }
                                    }

                                    // Display message
                                    MBDlgWinInsertCreate(-1, 0x3C22, D_80101C4C_shared_board[temp_s2][i], 0, 0, 0, 0);
                                    MBDlgWinClose();
                                    MBDlgWinKill();

                                    // Store item
                                    GwPlayer[system->current_player_index].bonusCoin = D_80101C2C_shared_board[temp_s2][i];

                                    // Display item sprite with animation
                                    temp_s2_2 = HuSprGrpCreate(1, 5);
                                    temp_v0_21 = DataRead(D_8010197C_shared_board[D_80101C2C_shared_board[temp_s2][i]]);
                                    temp_s3_3 = func_80055810_main(temp_v0_21);
                                    DataClose(temp_v0_21);

                                    func_80055024_main(temp_s2_2, 0, temp_s3_3, 0);
                                    HuSprPriSet(temp_s2_2, 0, 0xA);
                                    func_800550F4_main(temp_s2_2, 0, 0);
                                    HuSprAttrSet(temp_s2_2, 0, 0x180C);
                                    func_80054904_main(temp_s2_2, 0, 0xA0, 0x5A);
                                    func_80055458_main(temp_s2_2, 0, 0);
                                    HuAudFXPlay(0x10);

                                    // Animate sprite appearing
                                    for (i = 0; i < 0xA; i++) {
                                        func_80055458_main(temp_s2_2, 0, (i * 25));
                                        HuPrcVSleep();
                                    }

                                    func_80055458_main(temp_s2_2, 0, 0x100);
                                    HuPrcSleep(5);
                                    MBPlayerMotionSet(-1, 5, 0);
                                    HuPrcSleep(5);
                                    func_8004ACE0_main(0x2BD, system->current_player_index);
                                    HuPrcSleep(0x14);
                                    func_800F2388_shared_board(-1, -1, 0, 5, 2);
                                    HuPrcSleep(0xA);

                                    // Clean up sprite
                                    HuSprGrpKill(temp_s2_2);
                                    HuSprKill(temp_s3_3);
                                    goto tempCase17;
                                }

                                MBDlgResultWinExec(0x3C32);
                                goto label2444;

                            label1d70:
                                func_800F7108_shared_board();

                                for (i = 0; i < 4; i++) {
                                    GwPlayer[i].group = 1;
                                }

                                temp_v0_4->group = 0;
                                HuPrcChildLink(telopProc, func_800E0F84_shared_board());
                                HuPrcChildWait();

                                mbGameMode = 0x11;
                                D_80105664_shared_board = 2;
                                D_80105666_shared_board = 1;
                                D_800D1360.unk_20 = 6;
                                HuPrcSleep(-1);
                                goto label2444;
                        }

                    tempCase17:

                        GwPlayer[system->current_player_index].itemNo[MBItemFindEmpty(system->current_player_index)] =
                            GwPlayer[system->current_player_index].bonusCoin;
                        HuPrcSleep(0x1E);
                        goto label2444;
                    case SPACE_BATTLE:
                        if (!(func_800FB624_shared_board(temp_v0_4))) {
                            goto label2444;
                        } else {
                            mbItemBtnF = 0;
                            func_800F7108_shared_board();
                            HuPrcChildLink(telopProc, MBMgCallBattleCreate());
                            HuPrcChildWait();
                            mbGameMode = 0x12;
                            D_80105664_shared_board = 2;
                            D_80105666_shared_board = 1;
                            D_800D1380_main = 8;
                            HuPrcSleep(-1);
                        }

                    test:
                        for (i = 0; i < 4; i++) {
                            if (GwPlayer[i].bonusCoin != 3) {
                                break;
                            }
                        }

                        if (i >= 4) {
                            // All players done with battle
                            func_800FC3D0_shared_board();
                        }

                        goto label2444;
                    case SPACE_BOWSER:
                        mbGameMode = 6;
                        GwSystem.playerIndexVisitingBowser = system->current_player_index;
                        func_800FF794_shared_board(0x50, 0, 3, 1);
                        HuPrcSleep(-1);
                        HuPrcSleep(0x1E);
                        goto label2444;

                    case SPACE_GAME_GUY:
                        HuPrcSleep(8);

                        // Create game guy object
                        temp_v0_22 = MBModelCreate(0x3E, NULL);
                        MBModelTempAllocFree(temp_v0_22);
                        HuVecCopy3F(&temp_v0_22->coords, &temp_v0_4->player_obj->coords);
                        temp_v0_22->velocity.x = 100.0f;

                        // Animate game guy appearing on screen
                        while (temp_v0_22->velocity.x >= 30.0f) {
                            temp_v0_22->velocity.x -= 2.0f;
                            HuPrcVSleep();
                        }

                        temp_s1 = func_800EDB98_shared_board(temp_v0_22, 5.0f, 2.0f);
                        HuAudFXPlay(0x2A1);

                        if (GwPlayer[system->current_player_index].coin != 0) {
                            // Take all coins
                            MBDlgWinExec(0xA, 0x2F00);
                            GwPlayer[system->current_player_index].gameCoin =
                                GwPlayer[system->current_player_index].coin;

                            MBCoinChangeCreate(system->current_player_index,
                                               -GwPlayer[system->current_player_index].coin);
                            MBCoinTakeCreate(system->current_player_index,
                                                              -GwPlayer[system->current_player_index].coin);
                            HuPrcSleep(0x1E);
                            MBDlgWinExec(0xA, 0x2F02);
                            omDelPrcObj(temp_s1);

                            // Animate game guy leaving with player
                            while (temp_v0_22->velocity.x >= 10.0f) {
                                temp_v0_22->velocity.x -= 2.0f;
                                HuPrcVSleep();
                            }

                            func_800EE688_shared_board(temp_v0_22, 2.0f, 0.0001f);
                            func_800EE688_shared_board(temp_v0_4->player_obj, 2.0f, 0.0001f);
                            HuPrcSleep(0x1E);
                        } else {
                            MBDlgWinExec(0xA, 0x2F01);
                            omDelPrcObj(temp_s1);
                            // Animate game guy leaving with player
                            while (temp_v0_22->velocity.x <= 100.0f) {
                                temp_v0_22->velocity.x += 4.0f;
                                HuPrcVSleep();
                            }
                            MBModelKill(temp_v0_22);
                            goto label2488;
                        }

                        if (MBPlayerComCheck(-1) != 0 && system->show_com_minigames != 0) {
                            // COM game guy - immediate result
                            WipeCreateOut(9, 0x10);
                            HuPrcSleep(0x11);
                            MBModelKill(temp_v0_22);
                            func_800EE688_shared_board(temp_v0_4->player_obj, 0, 0);
                            temp_v0_4->player_obj->velocity.x = 0.0f;
                            HuPrcSleep(5);
                            WipeCreateIn(9, 0x10);
                            HuPrcSleep(0x11);

                            // Weighted random multiplier
                            temp_v1_8 = MBRand(GAME_GUY_RANDOM_MAX) + 1;

                            for (i = 0; i < ARRAY_COUNT(D_80101C08_shared_board); i++) {
                                if (D_80101C08_shared_board[i].weight >= temp_v1_8) {
                                    break;
                                }
                            }

                            temp_a0_3 = D_80101C08_shared_board[i].multiplier;

                            if (temp_a0_3 != 0) {
                                // Won multiplier
                                var_s1_12 = GwPlayer[system->current_player_index].gameCoin * temp_a0_3;

                                if (var_s1_12 > 999) {
                                    var_s1_12 = 999;
                                }

                                sprintf(sp48, "%d", var_s1_12);
                                MBDlgWinInsertCreate(-1, 0x2F03, (s32)sp48, 0, 0, 0, 0);
                                MBDlgWinClose();
                                MBDlgWinKill();

                                MBCoinChangeCreate(system->current_player_index, var_s1_12);
                                MBCoinTakeCreate(system->current_player_index, var_s1_12);
                                HuPrcSleep(0x14);
                                MBPlayerMotionSet(-1, 5, 0);
                                func_8004ACE0_main(0x274, system->current_player_index);
                                MBPlayerMotionWait(-1);
                            } else {
                                // Lost everything
                                MBDlgWinExec(-1, 0x2F04);
                                MBPlayerMotionSet(-1, 3, 0);
                                func_8004ACE0_main(0x286, system->current_player_index);
                                MBPlayerMotionWait(-1);
                                HuPrcSleep(0x14);
                            }

                            func_800F2388_shared_board(-1, -1, 0, 5, 2);
                            HuPrcSleep(0xA);
                            goto label2488;
                        } else {
                            // Setup for game guy minigame
                            mbGameMode = 0x13;
                            D_80105664_shared_board = 2;
                            D_80105666_shared_board = 1;
                            D_800D1360.unk_20 = 9;

                            for (i = 0; i < 4; i++) {
                                GwPlayer[i].group = 1;
                            }
                            temp_v0_4->group = 0;

                            // Select minigame based on weighted random
                            temp_a0_4 = MBRand(100.0f);
                            var_v1_3 = 0;

                            for (var_v1_3 = 0; var_v1_3 < ARRAY_COUNT(D_80101C04_shared_board); var_v1_3++) {
                                if (D_80101C04_shared_board[var_v1_3] > temp_a0_4) {
                                    break;
                                }
                            }

                            if (var_v1_3 == ARRAY_COUNT(D_80101C04_shared_board)) {
                                var_v1_3 = ARRAY_COUNT(D_80101C04_shared_board) - 1;
                            }

                            system->minigame_index = D_80101C00_shared_board[var_v1_3];
                            GwSystem.playerIndexVisitingBowser = system->current_player_index;
                        }

                        HuPrcSleep(-1);
                        goto label2438;

                    case SPACE_UNK:
                    case SPACE_BANK:
                    case SPACE_START:
                    case SPACE_UNK2: // Mushroom space? (from mp1)
                    case SPACE_UNK3:
                    case SPACE_ARROW:
                    case SPACE_STAR:
                    default:
                        goto label2444;
                }
            case 17:
                goto tempCase17;
            case 18:
                goto test;
            case 19:
            label2438:
                func_800FC4E0_shared_board();
            label2444:
                mbGameMode = 7;
                D_80105660_shared_board = MBMasuEventCall(var_s5, 5);
                if (D_80105664_shared_board < 0) {
                    goto label2488;
                }
                HuPrcSleep(-1);
            case 7:
                HuPrcSleep(0x1E);
            label2488:
                mbItemBtnF = 0;

                // END OF TURN: Check for end of round minigames
                if (system->current_player_index >= 3) {
                    MBMasuEventCall(-3, 7);
                    func_8001FDE8_main(MBPlayerGet(3)->player_obj->omObj1->model[0]);
                    D_800A12D4_main = 0;
                    func_800F7108_shared_board();

                    HuPrcChildLink(telopProc, MBMgCallCreate());
                    HuPrcChildWait();

                    if (system->minigame_index >= 0) {
                        mbGameMode = 0;

                        // Determine minigame type
                        if (system->total_turns >= 0 && system->current_turn >= system->total_turns) {
                            if (GwSystem.wackyWatchUsedState == WATCH_USED) {
                                D_80105664_shared_board = 3;
                            } else {
                                D_80105664_shared_board = 0x23;
                            }
                        } else {
                            D_80105664_shared_board = 3;
                        }
                        D_80105666_shared_board = 1;
                        D_800D1360.unk_20 = 0xA;

                        for (var_s0_2 = 0; var_s0_2 < 4; var_s0_2++) {
                            func_800F482C_shared_board(var_s0_2);
                        }

                        HuPrcSleep(-1);
                    }
                }

                // Check for game end condition
                if (MBLast5Check() != 0) {
                    mbGameMode = 0;
                    D_80105664_shared_board = 1;
                    HuPrcSleep(-1);
                }

                HuPrcSleep(4);

                HuPrcChildLink(telopProc, func_800F292C_shared_board());
                HuPrcChildWait();

                MBNextPlayerTurn();

                // Hide all players
                for (i = 0; i < 4; i++) {
                    MBPlayerForwardSet(i);
                    MBMotionSet(MBPlayerGet(i)->player_obj, -1, 2);
                }

                HuPrcVSleep();

                if (GwSystem.current_board_index == CHILLY_WATERS && GWBoardFlagCheck(0x12) != 0) {
                    MBMasuDispOn();
                    GWBoardFlagClear(0x12);
                }

                func_800FF840_shared_board(NULL);
                mbGameMode = 1;
        }
    }
}

static void func_800FF0F8_shared_board(void) {
    HuPrcVSleep();
    MBMapScrollExec(GwPlayer[GwSystem.current_player_index].pad);
    MBExit();
    omOvlReturnEx(1);
    omOvlKill();
    HuPrcVSleep();
}

static void func_800FF158_shared_board(void) {
    Vec2f sp10;
    s32 curPlayerNo;
    s32 i;
    s32 comDecision;
    s32 playerPadNo;

    while (1) {
        HuPrcVSleep();
        if (WipeStatGet() != 0) {
            continue;
        }
        if (mbItemBtnF == 0) {
            continue;
        }
        if (D_800CC0BA_main == 3) {
            continue;
        }
        curPlayerNo = MBPlayerTurnGet();
        for (i = 0; i < MB_MAX_PLAYERS; i++) {
            if ((D_800C9520_main[i] & START_BUTTON) && GwPlayer[GwSystem.current_player_index].itemTurn == 0) {
                func_800DE874_shared_board(curPlayerNo);
                MBCameraPos2DGet(&sp10);
                MBCameraPos2DSet(&sp10);
                MBPauseCreate(i);
                func_800DE8D8_shared_board(curPlayerNo);
                break;
            }
            if (MBPlayerComCheck(i) == 0) {
                playerPadNo = GwPlayer[i].pad;
                if (i == curPlayerNo) {
                    if (D_800CDD64_main != 0 && (D_800C9520_main[playerPadNo] & B_BUTTON)) {
                        if (!(GwSystem.cur_player_used_item & 1)) {
                            func_800FC8A4_shared_board();
                            func_800DE874_shared_board(i);
                            MBItemSelExec(i, -1);
                            func_800DE8D8_shared_board(i);
                        }
                        break;
                    }
                    if (D_800D51F8_main != 0 && (D_800C9520_main[playerPadNo] & B_BUTTON)) {
                        func_800DE874_shared_board(curPlayerNo);
                        MBItemViewExec(curPlayerNo);
                        func_800DE8D8_shared_board(curPlayerNo);
                        break;
                    }
                    if (mbCameraBtnF != 0 && (D_800C9520_main[playerPadNo] & R_TRIG)) {
                        func_800DE874_shared_board(curPlayerNo);
                        MBMapScrollExec(playerPadNo);
                        func_800DE8D8_shared_board(curPlayerNo);
                        break;
                    }
                    if (mbCameraBtnF != 0 && (D_800C9520_main[playerPadNo] & Z_TRIG)) {
                        func_800DE874_shared_board(curPlayerNo);
                        MBMapFullExec(playerPadNo);
                        func_800DE8D8_shared_board(curPlayerNo);
                        break;
                    }
                }
            } else if (D_800CDD64_main != 0 && D_800CC4A0_main != 0) {
                if (i == curPlayerNo) {
                    comDecision = MBComItemDecide();
                    if (comDecision == -1) {
                        D_800CC4A0_main = 0;
                    } else {
                        func_800FC8A4_shared_board();
                        func_800DE874_shared_board(curPlayerNo);
                        MBItemSelExec(curPlayerNo, comDecision);
                        func_800DE8D8_shared_board(curPlayerNo);
                    }
                }
            }
        }
    }
}

void MBStart(u32 arg0) {
    GW_PLAYER *player;
    s32 i;

    for (i = 0; i < MB_MAX_PLAYERS; i++) {
        s16 masuID;
        player = MBPlayerGet(i);
        masuID = MBMasuLinkMasuIdGet(player->clink, player->cidx);
        MBPlayerPosMasuSet(i, masuID);
    }

    func_800E6FBC_shared_board();
    func_800E9730_shared_board(1.0f);
    MBCameraSpeedSet(-1.0f);
    D_800CD280_main = 1;
    switch (arg0) {
        case 0:
            switch (mbGameMode) {
                case 0:
                case 1:
                    WipeCreateIn(0, 0);
                    func_800F2834_shared_board();
                    break;
                default:
                    if (D_800A12C8_main != -1) {
                        if (D_800A12C8_main == 0) {
                            WipeColorSet(255, 255, 255);
                        } else {
                            WipeColorSet(0, 0, 0);
                        }
                        WipeCreateIn(D_800A12C8_main, 0x10);
                        D_800A12C8_main = -1;
                        MBPlayerVibrate(0, 0);
                        MBPlayerVibrate(1, 0);
                        MBPlayerVibrate(2, 0);
                        MBPlayerVibrate(3, 0);
                    } else {
                        WipeCreateIn(0xFF, 0x10);
                    }
                    MBCameraZoomSet(D_800C9938_main);
                    func_800F27D0_shared_board();
                    break;
            }
            MBCameraPos3DSet(&GwPlayer[GwSystem.current_player_index].player_obj->coords);
            omAddPrcObj(MBMain, 0xEFFF, 0x3000, 0);
            omAddPrcObj(func_800F93A4_shared_board, 0x1005, 0, 0);
            omPrcSetStatBit(omAddPrcObj(func_800FF158_shared_board, 0x1001, 0, 0), 0x80);
            mbCameraBtnF = 0;
            mbItemBtnF = 0;
            D_800CDD64_main = 0;
            D_800D51F8_main = 0;
            omAddPrcObj(func_800F91A4_shared_board, 0x1005, 0, 0);
            MBCameraFocusModeSet(1);
            if (func_800037C4_main(D_800CDBC8_main) < 0x7F) {
                func_800DECBC_shared_board();
                return;
            }
            return;
        case 1:
            MBCameraPos3DSet(&GwPlayer[GwSystem.current_player_index].player_obj->coords);
            for (i = 0; i < MB_MAX_PLAYERS; i++) {
                MBStatusColorSet(i, MBPlayerGet(i)->color);
            }
            omAddPrcObj(func_800F93A4_shared_board, 0x1005, 0, 0);
            omPrcSetStatBit(omAddPrcObj(func_800FF0F8_shared_board, 0x1005, 0, 0), 0x80);
            mbCameraBtnF = 1;
            return;
        case 2:
            MBStatusHideAll();
            MBCameraFocusModeSet(0);
            break;
    }
}

void func_800FF75C_shared_board(s16 arg0) {
    D_800CD2A0_main.unk_02 = 1;
    D_80105664_shared_board = 0x40;
    D_800D1380_main = arg0;
    D_80105666_shared_board = 0;
}

void func_800FF788_shared_board(s32 arg0) {
    D_800A12C8_main = arg0;
}

void func_800FF794_shared_board(s32 arg0, s32 arg1, s32 arg2, s32 arg3) {
    D_80105668_shared_board = arg0;
    D_8010566C_shared_board = arg1;
    D_80105664_shared_board = 8;
    D_800D1380_main = arg2;
    D_80105666_shared_board = arg3;
}

void func_800FF7C4_shared_board(s32 arg0, s16 arg1, s16 arg2) {
    func_800FF794_shared_board(arg0, arg1, arg2, 0);
}

void func_800FF7F0_shared_board(s32 arg0) {
    D_800D530C_main = 1;
}

void func_800FF800_shared_board(s16 arg0, s16 arg1) {
    D_80105664_shared_board = 0x10;
    D_800D1380_main = arg0;
    D_80105666_shared_board = arg1;
    func_800F884C_shared_board();
}

s16 func_800FF834_shared_board(void) {
    return D_80105664_shared_board;
}

void func_800FF840_shared_board(Vec *arg0) {
    f32 temp_f20;
    f32 temp_f22;

    func_800E728C_shared_board();
    func_800E7254_shared_board();
    temp_f22 = MBCameraSpeedGet();
    MBCameraSpeedSet(-1.0f);
    temp_f20 = func_800E973C_shared_board();
    func_800E9730_shared_board(1.0f);
    func_800E6FBC_shared_board();

    if (arg0 == NULL) {
        arg0 = &MBPlayerGet(CUR_PLAYER)->player_obj->coords;
    }

    MBCameraPos3DSet(arg0);
    HuPrcVSleep();
    func_800E9730_shared_board(temp_f20);
    MBCameraSpeedSet(temp_f22);
}
