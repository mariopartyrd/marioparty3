...
extern void func_800354A0_main(void *arg);
...
void func_800357AC_main(s32);

void ultraMain() {
    __osInitialize_common();
    __osInitialize_autodetect();
    InitSchedulerEvent();
    func_800357AC_main(3);
    osCreateThread(&gInitThread, 1, &thread1_idle, NULL, &gThread1Stack[2048], 10);
    osStartThread(&gInitThread);
}

void thread1_idle(void *arg) {
    osCreateThread(&gMainThread, 3, &func_800354A0_main, arg, &gThread3Stack, 2);
